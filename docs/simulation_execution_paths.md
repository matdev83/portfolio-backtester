...
1. `calculate_portfolio_returns` (non-meta) builds [`PortfolioSimulationInput`](../src/portfolio_backtester/backtester_logic/portfolio_simulation_input.py): dense targets, **`rebalance_mask`** (from sparse execution rows or an explicit override), **`ledger_decision_idx`** (per-asset audit decision row for ledger fills; equals the execution row for `bar_close`, and for `next_bar_open` is derived alongside invalid-open mask propagation), close panel, and optional **open** panel when `trade_execution_timing=next_bar_open`.
