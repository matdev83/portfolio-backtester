...
def lookup_openfigi(
    api_key: str, ticker: str, *, throttle: float = 1.0
        ...
        try:
            ...
            if response_data and isinstance(response_data, list) and response_data[0].get("data"):
                for security_info in response_data[0]["data"]:
                    ...
                    name = (
                        security_info.get("securityDescription")
                        or security_info.get("name")
                        or security_info.get("securityName")
                    )
                ...
                response_data and isinstance(response_data, list) and "warning" in response_data[0]
            ):
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(
                        f"OpenFIGI warning for {ticker} with payload {payload}: {response_data[0]['warning']}"
                    )
            ...
            logger.error(
                f"Error processing OpenFIGI response for {ticker} with payload {payload}: {e}"
            )
...
def _query_openfigi_api(api_key: str, url: str, payload: list) -> Optional[list]:
    ...
    try:
        ...
        if isinstance(result, list):
            ...
            logger.warning(f"OpenFIGI API returned non-list JSON response: {result}")
            return None
    except requests.exceptions.Timeout:
        logger.warning(f"OpenFIGI API request timed out for payload: {payload}")
    except requests.exceptions.HTTPError as http_err:
        logger.warning(
            f"OpenFIGI API HTTP error for payload {payload}: {http_err.response.status_code} - {http_err.response.text}"
        )
    except requests.RequestException as req_err:
        logger.warning(f"OpenFIGI API request failed for payload {payload}: {req_err}")
    except json.JSONDecodeError as json_err:
        logger.error(
            f"Failed to decode JSON response from OpenFIGI for payload {payload}: {json_err}"
        )
    return None
