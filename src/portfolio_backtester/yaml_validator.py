...
class YamlValidator:
    ...
    def validate_file(self, file_path: Union[str, Path]) -> Tuple[bool, Optional[Dict[str, Any]], List[YamlError]]:
        """
    ...
    def _validate_yaml_content(self, content: str, file_path: str) -> Tuple[bool, Optional[Dict[str, Any]], List[YamlError]]:
        """
    ...
    def _parse_yaml_error(self, yaml_error: yaml.YAMLError, content: str, file_path: str) -> YamlError:
        """
    ...
    def _generate_suggestion(self, yaml_error: yaml.YAMLError, lines: List[str], line_idx: int, col_idx: int) -> str:
        """
        ...
        if "mapping values are not allowed here" in error_str:
            return ("This usually means there's an indentation problem or missing colon. "
            ...
            return ("The YAML file ended unexpectedly. Check for unclosed brackets, braces, or quotes.")
            ...
            return ("Invalid YAML value. Check for unquoted special characters or malformed data types.")
        
        elif "scanner error" in error_str:
            if "'" in error_line or '"' in error_line:
                return ("Quote character issue. Make sure all quotes are properly closed and escaped.")
            else:
                return ("Character encoding or special character issue. Check for invalid characters.")
        ...
        return ("Check the YAML syntax around this line. Common issues include: "
    ...
    def _generate_general_suggestion(self, yaml_error: yaml.YAMLError) -> str:
        ...
        if "scanner error" in error_str:
            return ("Scanner error usually indicates invalid characters or encoding issues. "
                   "Check for special characters, ensure UTF-8 encoding, and verify quote matching.")
        
        elif "parser error" in error_str:
            return ("Parser error indicates structural YAML issues. "
                   "Check overall file structure, indentation consistency, and bracket/brace matching.")
        
        else:
            return ("Please check the YAML file for common syntax issues: "
    ...
    def format_errors(self, errors: List[YamlError]) -> str:
        ...
        for i, error in enumerate(errors, 1):
            ...
            if error.line_number:
                formatted_lines.append(f"  Location: Line {error.line_number}, Column {error.column_number}")
