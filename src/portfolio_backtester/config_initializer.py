...
def _get_strategy_tunable_params(strategy_spec) -> Set[str]:
    ...
    return parameter_extractor_factory.extract_parameters(strategy_spec)
...
def populate_default_optimizations(scenarios: list, optimizer_parameter_defaults: dict):
    ...
    for scenario_config in scenarios:
        ...
        # Use polymorphic optimization populator to extract parameters from both legacy and modern formats
        optimization_populator_factory = OptimizationPopulatorFactory()

        # Extract optimized parameters using polymorphic approach
        legacy_params = {opt_spec["parameter"] for opt_spec in scenario_config["optimize"]}
        modern_params = optimization_populator_factory.extract_optimized_parameters(scenario_config)
        optimized_parameters_in_scenario = legacy_params.union(modern_params)

        # Populate strategy_params with defaults from modern format
        ...
        for param_name in optimized_parameters_in_scenario:
            if param_name not in valid_params and param_name not in optimizer_parameter_defaults:
                ...
                print(
                    f"Warning: Parameter '{param_name}' in scenario '{scenario_config.get('name', 'unnamed')}' "
                    f"is not tunable by strategy '{strategy_name}'. It will be skipped during optimization."
                )

        if invalid_params:
            print(
                f"Warning: Scenario '{scenario_config.get('name', 'unnamed')}' contains invalid parameters "
        ...
        if len(original_optimize_list) > 0:
            ...
            for param_name in optimized_parameters_in_scenario:
                if (
                    param_name not in scenario_config["strategy_params"]
                    and param_name in optimizer_parameter_defaults
                ):
