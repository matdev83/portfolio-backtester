...
def run_backtest_mode(self, scenario_config, monthly_data, daily_data, rets_full):
    ...
    if self.args.study_name:
        try:
            ...
            if self.logger.isEnabledFor(logging.DEBUG):
                self.logger.debug(
                    f"Loaded best parameters from study '{self.args.study_name}': {optimal_params}"
                )
        except KeyError:
            self.logger.warning(
...
def run_optimize_mode(self, scenario_config, monthly_data, daily_data, rets_full):
    ...
    optimal_params, actual_num_trials, best_trial_obj = self.run_optimization(
        scenario_config, monthly_data, daily_data, rets_full
    )
    ...
    if optimal_params is None:
        self.logger.error(
            f"Optimization for {scenario_config['name']} did not yield optimal parameters. Skipping full backtest."
        )
        return
    ...
    if self.logger.isEnabledFor(logging.DEBUG):
        self.logger.debug(
            f"Full backtest with optimized parameters completed for {scenario_config['name']}."
        )
    ...
    if enable_optimization_reports:
        if enable_during_optimization:
            ...
            if self.logger.isEnabledFor(logging.DEBUG):
                self.logger.debug(
                    "⚡ Optimization report generation deferred for performance - will generate after optimization completes"
                )
