...
def track_trades_vectorized(
    ...
    # Align data
    common_dates = weights_daily.index.intersection(close_prices.index)
    if len(common_dates) == 0:
        logger.warning("No common dates between weights and prices")
        return _get_empty_trade_stats()
    
    weights_aligned = weights_daily.reindex(common_dates).fillna(0.0)
    prices_aligned = close_prices.reindex(common_dates).ffill()
    costs_aligned = transaction_costs.reindex(common_dates).fillna(0.0)
    ...
    position_values, position_changes, trade_costs, margin_usage = _calculate_trade_metrics(
        weights_array, prices_array, costs_array, portfolio_value
    )
    ...
    total_trades, total_turnover, total_costs, max_margin, mean_margin, avg_trade_cost = _calculate_basic_trade_stats_vectorized(
        ...
        trade_costs, margin_usage, portfolio_value
    )
...
def _calculate_basic_trade_stats_vectorized(
    ...
    max_margin = np.max(margin_usage) if len(margin_usage) > 0 else 0.0
    mean_margin = np.mean(margin_usage) if len(margin_usage) > 0 else 0.0
    
    # Calculate average trade cost
    avg_trade_cost = total_costs / max(total_trades, 1.0)
