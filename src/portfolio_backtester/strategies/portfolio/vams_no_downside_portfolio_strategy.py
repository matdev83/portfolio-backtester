...
class VamsNoDownsidePortfolioStrategy(PortfolioStrategy):
    def _calculate_candidate_weights(self, scores: pd.Series) -> pd.Series:
        params = self.strategy_config.get("strategy_params", self.strategy_config)
        return cast(pd.Series, default_candidate_weights(scores, params))
    ...
    def _apply_leverage_and_smoothing(
        ...
        params = self.strategy_config.get("strategy_params", self.strategy_config)
        return cast(
            pd.Series, apply_leverage_and_smoothing(candidate_weights, prev_weights, params)
        )
    ...
    def _calculate_vams_scores_no_downside(
        self, asset_prices: pd.DataFrame, lookback_months: int, current_date: pd.Timestamp
    ) -> pd.Series:
        """
        ...
        try:
            # Use polymorphic interface for current price extraction
            price_now = extract_current_prices(relevant_prices, current_date, asset_prices.columns)

            # Get lookback date and extract price using polymorphic interface
            lookback_date = current_date - pd.DateOffset(months=lookback_months)
            price_l_months_ago = extract_current_prices(
                relevant_prices, lookback_date, asset_prices.columns
            )

            if price_now.empty or price_l_months_ago.empty:
                return pd.Series(dtype=float, index=asset_prices.columns)
    ...
    def generate_signals(
        ...
        # --- Prepare Data: extract and normalize asset prices ---
        # Extract asset prices using the same pattern as VAMS momentum strategy
        if (
        ...
        scores_at_current_date = self._calculate_vams_scores_no_downside(
            asset_prices_hist, params["lookback_months"], current_date
        )
        ...
        if scores_at_current_date.isna().all() or scores_at_current_date.empty:
            ...
            # Extract current prices for entry logic using polymorphic interface
        ...
        stop_levels = sl_handler.calculate_stop_levels(
            current_date, asset_ohlc_hist_for_sl, self.w_prev, self.entry_prices
        )
        ...
        if sma_filter_window and sma_filter_window > 0:
            # Extract benchmark price series using the same pattern as VAMS momentum strategy
            ...
            # Extract current benchmark price and SMA - handle Series data manually
            if (
                ...
                current_benchmark_price = pd.Series(dtype=float)
                current_benchmark_sma = pd.Series(dtype=float)

            if not current_benchmark_price.empty and not current_benchmark_sma.empty:
                ...
                if self.current_derisk_flag:
                    final_weights[:] = 0.0
                elif float(current_benchmark_price.iloc[0]) < float(current_benchmark_sma.iloc[0]):
                    final_weights[:] = 0.0
