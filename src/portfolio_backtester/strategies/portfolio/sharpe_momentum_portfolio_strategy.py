...
from ...numba_optimized import sharpe_fast_fixed
from ...utils.price_data_utils import normalize_price_series_to_dataframe, extract_current_prices
...
class SharpeMomentumPortfolioStrategy(BaseMomentumPortfolioStrategy):
    ...
    def _calculate_scores(
        ...
        # Use the fixed Sharpe calculation for consistent results
    ...
    def generate_signals(
        ...
        **kwargs: Any,
        ...
        # Prepare asset data (universe) using polymorphic approach
        if (
            ...
            asset_data_for_scores = (
                all_historical_data  # Assuming it's already 'Close' prices or single field
            )
        ...
        if sma_filter_window and sma_filter_window > 0:
            ...
            if (
                ...
                if self.current_derisk_flag or (
                    ...
                    logger.debug(
                        f"[SharpeMomentumPortfolioStrategy] SMA filter zeroed weights on {current_date}"
                    )
                    final_weights[:] = 0.0
        ...
        if risk_off_generator:
            ...
            logger.debug(
                f"[SharpeMomentumPortfolioStrategy] Risk-off signal for {current_date}: {risk_off_signal}"
            )
            if risk_off_signal:  # True means risk-off conditions detected
                logger.debug(
                    f"[SharpeMomentumPortfolioStrategy] Risk-off filter zeroed weights on {current_date}"
                )
                final_weights[:] = 0.0
