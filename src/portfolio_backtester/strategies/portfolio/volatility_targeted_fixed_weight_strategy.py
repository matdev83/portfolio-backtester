...
import pandas as pd
import numpy as np
from .fixed_weight_strategy import FixedWeightStrategy

class VolatilityTargetedFixedWeightStrategy(FixedWeightStrategy):
    """
    Extends the FixedWeightStrategy to include volatility targeting.
    ...
    def __init__(self, strategy_params: Dict[str, Any]):
        ...
        self.vol_window = self.strategy_params.get("vol_window", 126)  # Default to 6 months (approx. 126 trading days)
    ...
    def generate_signals(self, all_historical_data: pd.DataFrame, **kwargs) -> pd.DataFrame:
        ...
        portfolio_returns = (daily_returns[base_weights.index] * base_weights).sum(axis=1)
    ...
    def tunable_parameters(cls) -> set:
        ...
        """
        return {"target_volatility", "vol_window"}
