...
from ...interfaces.strategy_registry_interface import (
    IStrategyRegistry,
...
class AutoDiscoveryStrategyRegistry(IStrategyRegistry):
    ...
    def add_strategy(self, name: str, strategy_class: Type[Any]) -> None:
...
class FileSystemStrategyDiscoveryEngine(IStrategyDiscoveryEngine):
    ...
    def discover_strategies(self) -> Dict[str, Type[Any]]:
        ...
        from portfolio_backtester.strategies import registry

        current_dir = os.path.dirname(registry.__file__)
        ...
        for subdir in self._discovery_paths:
            if subdir == "testing":
                ...
                if os.path.isdir(subdir_path):
                    for module_info in pkgutil.walk_packages(
                        [subdir_path], prefix=f"{package_name}.{subdir}."
                    ):
                        try:
                            importlib.import_module(module_info.name)
...
class ConcreteStrategyValidator(IStrategyValidator):
    ...
    def get_base_strategy_types(self) -> List[Type[Any]]:
        """Get base strategy types that should be excluded."""
...
class StrategyRegistryFactory:
    ...
    @staticmethod
    def create_custom_registry(discovery_paths: List[str]) -> IStrategyRegistry:
        """Create registry with custom discovery paths."""
        return StrategyRegistryFactory.create_registry(discovery_paths=discovery_paths)
