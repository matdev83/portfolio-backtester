...
class StrategyRegistry:
    ...
    def get_strategy_class(self, name: str) -> Optional[Type]:
        ...
        result = self._solid_registry.get_strategy_class(name)
        return result if isinstance(result, type) or result is None else None

    def get_all_strategies(self) -> Dict[str, Type]:
        """Get all discovered strategies."""
        result = self._solid_registry.get_all_strategies()
        return cast(Dict[str, Type], dict(result) if isinstance(result, dict) else {})

    def is_strategy_registered(self, name: str) -> bool:
        """Check if a strategy is registered."""
        return bool(self._solid_registry.is_strategy_registered(name))
