...
class BaseMetaStrategy(BaseStrategy, ABC):
    ...
    def _create_strategy_instance(
        ...
        logger.debug(f"Creating sub-strategy instance: {strategy_class} with params: {strategy_params}")
        ...
        try:
            ...
            return StrategyFactory.create_strategy(strategy_class, processed_params)
            ...
            # Find closest match
            import difflib
            matches = difflib.get_close_matches(strategy_class, available_strategies, n=3, cutoff=0.6)
    ...
    def generate_signals(
        ...
        for strategy_id, strategy in sub_strategies.items():
            try:
                ...
                logger.error(f"Error generating signals for sub-strategy {strategy_id}: {e}", exc_info=True)
