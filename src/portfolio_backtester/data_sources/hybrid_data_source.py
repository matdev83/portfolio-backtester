...
class HybridDataSource(BaseDataSource):
    ...
    def __init__(
        ...
        prefer_stooq: bool = True,
        negative_cache_timeout_hours: int = 4,
    ) -> None:
        """
        ...
        self.failed_tickers: Dict[str, Set[str]] = {"stooq": set(), "yfinance": set()}
        self._negative_cache: Dict[str, Dict[Tuple[str, str], datetime]] = {}
        self._load_negative_cache()

        logger.debug(
            f"HybridDataSource initialized. Data directory: {self.data_dir}, prefer_stooq: {self.prefer_stooq}"
        )
    ...
    def _is_data_valid(self, df: pd.DataFrame, ticker: str, min_rows: int = 3) -> bool:
        ...
        if len(df) < min_rows:
            logger.debug(
                f"Data validation failed for {ticker}: Only {len(df)} rows (minimum: {min_rows})"
            )
            return False

        # Check for valid date index using polymorphic validation
        from ..interfaces.data_validator_interface import create_data_validator

        validator = create_data_validator()

        if not validator.is_valid_datetime_index(df):
            logger.debug(f"Data validation failed for {ticker}: Index is not DatetimeIndex")
            return False

        # Check for required columns using polymorphic validation
        is_valid, error_msg = validator.validate_ticker_data_structure(df, ticker, "Close")
        if not is_valid:
            logger.debug(f"Data validation failed for {ticker}: {error_msg}")
            return False

        # Extract close prices for validation using polymorphic extractor
        try:
            from ..interfaces.price_extractor_data_source import create_price_extractor

            extractor = create_price_extractor()
            close_series = extractor.extract_close_price(df, ticker)
        except KeyError:
        ...
        if nan_ratio > 0.5:
            logger.debug(
                f"Data validation failed for {ticker}: Too many NaN values ({nan_ratio:.1%})"
            )
            return False
        ...
        logger.debug(
            f"Data validation passed for {ticker}: {len(df)} rows, {nan_ratio:.1%} NaN ratio"
        )
        return True
    ...
    def _repair_uvxy_data(self, df: pd.DataFrame, ticker: str) -> pd.DataFrame:
        ...
        logger.info(
            f"Detected {len(jump_dates)} sharp price jumps in {ticker} data, applying repairs..."
        )
        ...
        for jump_date in sorted_jump_dates:
            ...
            if jump_ratio > 3.0:
                ...
                logger.debug(
                    f"Applied upward jump repair on {jump_date}: "
                ...
                logger.debug(
                    f"Applied downward jump repair on {jump_date}: "
        ...
        if remaining_jumps > 0:
            ...
            logger.info("UVXY repair successful: price continuity restored")
        ...
        logger.info("UVXY repair summary:")
    ...
    def _normalize_data_format(
        self, df: pd.DataFrame, source: str, tickers: List[str]
    ) -> pd.DataFrame:
        """
    ...
    def _convert_to_multiindex(self, df: pd.DataFrame, tickers: List[str]) -> pd.DataFrame:
        ...
        for ticker in tickers:
            if ticker in df.columns:
                ...
                ticker_data = pd.DataFrame({"Close": df[ticker]})
                ...
                ticker_data.columns = pd.MultiIndex.from_product(
                    [[ticker], ticker_data.columns], names=["Ticker", "Field"]
                )
    ...
    def _fetch_from_source(
        self, source: str, tickers: List[str], start_date: str, end_date: str
        ...
        try:
            ...
            if not normalized_df.empty:
                ...
                normalizer = OHLCNormalizerFactory.create_normalizer(normalized_df)
                available_tickers = normalizer.get_available_tickers(normalized_df)

                for ticker in tickers:
                    if ticker in available_tickers:
                        try:
                            ticker_data = normalizer.extract_ticker_data(normalized_df, ticker)
            ...
            logger.info(
                f"{source} results: {len(successful_tickers)} successful, {len(failed_tickers)} failed"
            )
            if failed_tickers:
            ...
            if successful_tickers:
                try:
                    # Use normalizer to determine which columns to keep based on successful tickers
                    normalizer = OHLCNormalizerFactory.create_normalizer(normalized_df)

                    # Create a filtered DataFrame with only successful ticker data
                    filtered_data_frames = []
                    for ticker in successful_tickers:
                        try:
                            ticker_data = normalizer.extract_ticker_data(normalized_df, ticker)
                            filtered_data_frames.append(ticker_data)
                        except KeyError:
                            continue
                    ...
                    logger.warning(
                        f"Failed to filter successful tickers using normalizer, using fallback: {e}"
                    )
                    # Fallback to original approach if normalizer fails
                    if isinstance(normalized_df.columns, pd.MultiIndex):
                        cols_to_keep = [
                            col for col in normalized_df.columns if col[0] in successful_tickers
                        ]
                    else:
                        cols_to_keep = [
    ...
    def get_data(self, tickers: List[str], start_date: str, end_date: str) -> pd.DataFrame:
        ...
        for t in tickers:
            if t in self._negative_cache and (start_date, end_date) in self._negative_cache[t]:
                if (
                    now - self._negative_cache[t][(start_date, end_date)]
                    < self.negative_cache_timeout
                ):
                    cached_count += 1
                    continue
        ...
        if cached_count > 0:
            logger.info(
                f"Skipping {cached_count} tickers found in negative cache for the range {start_date}-{end_date}."
            )

        if not tickers_to_fetch:
            logger.info(
                "All requested tickers were in the negative cache for this range. No new data to fetch."
            )
            return pd.DataFrame()

        logger.info(
            f"Fetching data for {len(tickers_to_fetch)} tickers from {start_date} to {end_date}"
        )
        ...
        logger.info(
            f"Data fetching complete: {total_successful}/{len(tickers_to_fetch)} tickers successful"
        )

        if total_failed > 0:
            completely_failed = self.failed_tickers["stooq"] & self.failed_tickers["yfinance"]
            logger.warning(
                f"Failed to fetch data for {total_failed} tickers: {list(completely_failed)}"
            )
            ...
            logger.debug(
                f"Updated negative cache with {len(completely_failed)} tickers for the range {start_date}-{end_date}. Total size: {len(self._negative_cache)}"
            )
        ...
        logger.info(
            f"Source usage: {stooq_only} from Stooq only, {yfinance_only} from yfinance fallback"
        )
    ...
    def get_failure_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "yfinance_only_count": len(
                self.failed_tickers["stooq"] - self.failed_tickers["yfinance"]
            ),
            "complete_failure_count": len(
                self.failed_tickers["stooq"] & self.failed_tickers["yfinance"]
            ),
        }
