...
class YFinanceDataSource(BaseDataSource):
        ...
        if (
            ...
            try:
                ...
                if not isinstance(df.index, pd.DatetimeIndex) or df.empty:
                    if logger.isEnabledFor(logging.DEBUG):
                        logger.debug(
                            f"Cached file for {ticker} is empty or dates are invalid. Forcing re-download."
                        )
                    return None
                ...
                if logger.isEnabledFor(logging.DEBUG):
                    logger.debug(
                        f"Could not load {ticker} from cache ({e}). Cache might be corrupted."
                    )
        return None
    ...
    def _download_data(
        ...
        try:
            ...
            downloaded_df: pd.DataFrame = yf.download(
            ...
            for ticker in tickers:
                ...
                if df is not None and not df.empty:
                    ...
                    # Fallback for unusual cases or if data format from cache/download changes unexpectedly
                        ...
                        if logger.isEnabledFor(logging.WARNING):
                            logger.warning(
                                f"'Close' column not found for {ticker} despite auto_adjust=True. Using 'Adj Close'."
                            )
                        close_series = df["Adj Close"]
                    elif ticker in df.columns:  # Should not happen with yf.download structure
                        if logger.isEnabledFor(logging.WARNING):
                            logger.warning(
                                f"'Close' not found, using column named '{ticker}' for {ticker}."
                            )
                        close_series = df[ticker]
                    else:
                        logger.error(
                            f"Could not find 'Close' or 'Adj Close' column in downloaded/cached data for {ticker}"
                        )
