...
class MemoryDataSource:
    ...
    def get_data(
        ...
        if isinstance(self.daily_data.columns, pd.MultiIndex):
            ...
            return pd.DataFrame(self.daily_data.loc[mask, (selected_tickers, slice(None))])
        else:
            selected_tickers = [t for t in tickers if t in self.daily_data.columns]
            return pd.DataFrame(self.daily_data.loc[mask, selected_tickers])
    ...
    def get_benchmark_data(
        ...
        return pd.DataFrame(self.benchmark_data.loc[mask])
