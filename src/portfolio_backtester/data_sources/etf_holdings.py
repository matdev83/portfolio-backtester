...
class ETFHoldingsDataSource:
    ...
    def __init__(self, cache_dir: Optional[Path | str] = None, cache_expiry_days: int = 7):
    ...
    def _download_holdings(self, etf_ticker: str) -> pd.DataFrame | None:
        ...
        try:
            ...
            if "html" in response.headers.get("Content-Type", ""):
                logger.error(
                    f"Received HTML instead of CSV for {etf_ticker} holdings. Check URL or website."
                )
                return None
            ...
            if "Ticker" not in df.columns or "Weight" not in df.columns:
                logger.error(
                    f"Downloaded data for {etf_ticker} missing expected columns (Ticker, Weight). Available columns: {df.columns.tolist()}"
                )
                return None
            ...
            logger.error(
                f"An unexpected error occurred while downloading {etf_ticker} holdings: {e}"
            )
        return None
    ...
    def get_micro_cap_tickers(
        self, etf_ticker: str = "IWC", weight_threshold: float = 0.0001
    ) -> List[str]:
        """
        ...
        if holdings_df is None or holdings_df.empty:
            logger.warning(
                f"Could not retrieve holdings for {etf_ticker}. Returning empty micro-cap list."
            )
            return []

        # Filter by weight and return unique tickers
        micro_cap_tickers = (
            holdings_df[holdings_df["Weight"] > weight_threshold]["Ticker"].unique().tolist()
        )
        logger.info(
            f"Identified {len(micro_cap_tickers)} micro-cap tickers from {etf_ticker} holdings."
        )
        return list(micro_cap_tickers)
