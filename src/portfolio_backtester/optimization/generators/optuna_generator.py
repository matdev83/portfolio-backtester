...
class OptunaParameterGenerator(ParameterGenerator):
    ...
    def initialize(
        ...
        try:
            ...
            invalid_params = [name for name, cfg in self.parameter_space.items()
            ...
            for i, direction in enumerate(self.metric_directions):
                if direction not in ['maximize', 'minimize']:
                    logger.warning(f"Invalid direction '{direction}' for metric {i}. Using 'maximize'.")
            ...
            raise ParameterGeneratorError(f"Failed to initialize OptunaParameterGenerator: {e}") from e
    ...
    def _create_study(self, scenario_config: Dict[str, Any]) -> None:
        ...
        if self.study_name is None:
            ...
            self.study_name = f"{scenario_name}_optuna"
            if self.random_state is not None:
                self.study_name += f"_seed_{self.random_state}"
    ...
    def is_finished(self) -> bool:
        ...
        stop_flag = getattr(self.study, "_stop_flag", False)
    ...
    def get_best_result(self) -> OptimizationResult:
        ...
        try:
            ...
            best_value = -1e9 if not self.is_multi_objective else [-1e9] * len(self.metrics_to_optimize)
            return OptimizationResult(
                best_parameters={},
                best_value=best_value,
                n_evaluations=self.current_evaluation,
