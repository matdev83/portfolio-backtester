...
class GeneticTrialDeduplicator(AbstractTrialDeduplicator):
    ...
    def __init__(self, enable_deduplication: bool = True):
        ...
        self._base_deduplicator = GenericTrialDeduplicator(enable_deduplication)
    ...
    def add_trial(self, params: Dict[str, Any], result: Optional[float] = None) -> None:
        ...
        self._base_deduplicator.add_trial(params, result)

        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(f"Registered chromosome with fitness {result}: {params}")
...
class GeneticParallelRunner(AbstractParallelRunner):
    ...
    def evaluate_population_parallel(
        self, population: Any, fitness_function: Any, n_jobs: Optional[int] = 1
    ) -> Any:
        """
