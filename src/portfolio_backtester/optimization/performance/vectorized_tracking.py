...
class VectorizedTradeTracker(AbstractTradeTracker):
    ...
    def track_trades_optimized(
        self, weights: pd.DataFrame, prices: pd.DataFrame, costs: pd.Series
    ) -> Dict[str, Any]:
        """
        ...
        """
        # Always use optimized implementation
        ...
        weights_array: np.ndarray = weights_aligned.values.astype(np.float64)
        prices_array: np.ndarray = prices_aligned.values.astype(np.float64)
        costs_array: np.ndarray = costs_aligned.values.astype(np.float64)
