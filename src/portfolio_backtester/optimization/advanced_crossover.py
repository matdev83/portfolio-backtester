...
def simulated_binary_crossover(
    ...
    eta_c = _attribute_accessor.get_attribute(ga_instance, "sbx_distribution_index", 20.0)

    # Accumulate offspring as np.ndarray rows, then stack once
    offspring_rows: list[np.ndarray] = []
    idx = 0

    while len(offspring_rows) < offspring_size[0]:
        # Select two parents
        ...
        for i in range(len(parent1)):
            ...
            if abs(parent1[i] - parent2[i]) > 1e-14 and np.random.random() <= 0.5:
                ...
                if hasattr(ga_instance, "gene_space") and ga_instance.gene_space:
                    gene_space = (
                        ga_instance.gene_space[i]
                        if isinstance(ga_instance.gene_space, list)
                        else ga_instance.gene_space
                    )
        ...
        offspring_rows.append(parent1)
        if len(offspring_rows) < offspring_size[0]:
            offspring_rows.append(parent2)

        idx += 2

    # Trim to exact offspring size
    # Build final ndarray of desired shape
    offspring = np.array(offspring_rows[: offspring_size[0]], dtype=float)
    # Final clamping: ensure all offspring genes are within bounds
    if hasattr(ga_instance, "gene_space") and ga_instance.gene_space:
        for i in range(offspring.shape[0]):
            for j in range(offspring.shape[1]):
                gene_space = (
                    ga_instance.gene_space[j]
                    if isinstance(ga_instance.gene_space, list)
                    else ga_instance.gene_space
                )
                if isinstance(gene_space, dict):
...
def multi_point_crossover(
    ...
    # Dependency injection for attribute access (DIP)
    _attribute_accessor = attribute_accessor or create_attribute_accessor()
    num_points = _attribute_accessor.get_attribute(ga_instance, "num_crossover_points", 3)
    ...
    while len(offspring_rows) < offspring_size[0]:
        ...
        # Add offspring to population
        offspring_rows.append(offspring1)
        if len(offspring_rows) < offspring_size[0]:
            offspring_rows.append(offspring2)

        idx += 2

    # Trim to exact offspring size
    return np.array(offspring_rows[: offspring_size[0]], dtype=float)


def uniform_crossover_variant(
    parents: np.ndarray,
    offspring_size: Tuple[int, int],
    ga_instance: Any,
    attribute_accessor: Optional[IAttributeAccessor] = None,
    ...
    # Dependency injection for attribute access (DIP)
    _attribute_accessor = attribute_accessor or create_attribute_accessor()
    bias = _attribute_accessor.get_attribute(ga_instance, "uniform_crossover_bias", 0.5)
...
def arithmetic_crossover(
    parents: np.ndarray, offspring_size: Tuple[int, int], ga_instance: Any
) -> np.ndarray:
