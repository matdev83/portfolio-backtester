...
class PositionTracker:
    ...
    def update_positions(
        ...
        for ticker in signals.columns:
            raw_tw = signals_for_date[ticker]
            # Normalize raw value to a scalar float; treat non-scalar/NaN as 0.0
            try:
                # If raw_tw is a Series (e.g., due to duplicate columns), take the first non-null
                if isinstance(raw_tw, pd.Series):
                    scalar_candidate = (
                        raw_tw.dropna().iloc[0] if not raw_tw.dropna().empty else np.nan
                    )
                else:
                    scalar_candidate = raw_tw
                # Convert candidate to scalar float using polymorphic extractor
                from ..pandas_utils import extract_numeric_scalar

                scalar_value = extract_numeric_scalar(scalar_candidate)
                target_weight = scalar_value if scalar_value is not None else 0.0
            except Exception:
                target_weight = 0.0
            ...
            if current_position is None and abs(target_weight) > 1e-6:
                entry_price = (
                    self._get_price(prices, current_date, ticker) if prices is not None else None
                )
                ...
                logger.debug(
                    f"Opened position: {ticker} weight={target_weight:.4f} on {current_date.date()}"
                )
        ...
        try:
            if isinstance(prices.columns, pd.MultiIndex):
                ...
                if (ticker, "Close") in prices.columns and date in prices.index:
                    val = prices.loc[date, (ticker, "Close")]
                else:
                    return None
            else:
                # Single-level columns
                if ticker in prices.columns and date in prices.index:
                    val = prices.loc[date, ticker]
                else:
                    return None

            # Normalize to Python float using polymorphic extractor
            from ..pandas_utils import extract_numeric_scalar

            return extract_numeric_scalar(val)
    ...
    def get_daily_weights_df(self) -> pd.DataFrame:
        ...
        for weights in self.daily_weights:
            # Align to all columns and ensure float dtype
            aligned = weights.reindex(weights_df.columns, fill_value=0.0).astype(float)
            # Ensure the index label exists; if name is None, skip
    ...
    def get_trade_summary(self) -> Dict[str, float]:
        ...
        return {
            "total_trades": float(len(self.completed_trades)),
