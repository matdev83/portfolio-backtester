...
class SyntheticDataVisualInspector:
    ...
    def _perform_statistical_tests(
        ...
        synthetic_returns_list: List[pd.Series]
    ) -> Dict:
        """
        ...
        for i, synth_returns in enumerate(synthetic_returns_list):
            ...
            path_tests['mann_whitney'] = {'statistic': mw_stat, 'pvalue': mw_pvalue}
            
            # Levene test (for variance)
            lev_stat, lev_pvalue = stats.levene(historical_returns, synth_returns)
            path_tests['levene_test'] = {'statistic': lev_stat, 'pvalue': lev_pvalue}
    ...
    def _analyze_garch_properties(
        ...
        garch_analysis['historical'] = {
            'garch_params': hist_analysis.garch_params.__dict__ if hist_analysis.garch_params else None,
        ...
        for i, synth_returns in enumerate(synthetic_returns_list):
            try:
                ...
                synth_garch = {
                    'path_id': i + 1,
                    'garch_params': synth_analysis.garch_params.__dict__ if synth_analysis.garch_params else None,
    ...
    def _create_comprehensive_plots(
        ...
        # Calculate returns
        hist_returns = historical_data['Close'].pct_change(fill_method=None).dropna()
        synth_returns_list = [df['Close'].pct_change(fill_method=None).dropna() for df in synthetic_data]
        
        # Create main comparison plot
        self._create_main_comparison_plot(
            hist_returns, synth_returns_list, ticker, output_dir
        )
        ...
        self._create_autocorrelation_plots(
            hist_returns, synth_returns_list, ticker, output_dir
        )
        
        # Create price path comparison
        self._create_price_path_plots(
            historical_data, synthetic_data, ticker, output_dir
        )
    ...
    def _create_main_comparison_plot(
        ...
        for i, synth_returns in enumerate(synth_returns_list[:3]):  # Show first 3 paths
            ax1.plot(synth_returns.index, synth_returns, 
                    label=f'Synthetic {i+1}', alpha=0.6, linewidth=0.8)
        ...
        for i, synth_returns in enumerate(synth_returns_list[:3]):
            ax2.hist(synth_returns, bins=50, alpha=0.5, 
                    label=f'Synthetic {i+1}', density=True)
        ...
        stats.probplot(hist_returns, dist="norm", plot=ax3)
        ...
        if synth_returns_list:
            stats.probplot(synth_returns_list[0], dist="norm", plot=ax4)
        ...
        for i, synth_returns in enumerate(synth_returns_list[:3]):
            ...
            ax5.plot(synth_vol.index, synth_vol, 
                    label=f'Synthetic {i+1}', alpha=0.7)
        ...
        stats_data.append(['Historical'] + [f"{v:.4f}" if isinstance(v, (int, float)) else str(v) 
                                          for v in [hist_stats['mean'], hist_stats['std'], 
                                                   hist_stats['skewness'], hist_stats['kurtosis'],
                                                   hist_stats['var_95'], hist_stats['var_99']]])
        ...
        for i, synth_returns in enumerate(synth_returns_list[:5]):  # Show first 5 paths
            ...
            stats_data.append([f'Synthetic {i+1}'] + [f"{v:.4f}" if isinstance(v, (int, float)) else str(v)
                                                     for v in [synth_stats['mean'], synth_stats['std'],
                                                              synth_stats['skewness'], synth_stats['kurtosis'],
                                                              synth_stats['var_95'], synth_stats['var_99']]])
        
        columns = ['Data', 'Mean', 'Std', 'Skewness', 'Kurtosis', 'VaR 95%', 'VaR 99%']
        
        table = ax6.table(cellText=stats_data, colLabels=columns, 
        ...
        plt.suptitle(f'{ticker} - Comprehensive Synthetic Data Analysis', 
        ...
        if output_dir:
            ...
            plt.savefig(output_path / f'{ticker}_comprehensive_analysis.png', 
    ...
    def _create_distribution_plots(
        ...
        for i, synth_returns in enumerate(synth_returns_list[:3]):
            axes[0, 0].hist(synth_returns, bins=50, alpha=0.5, 
                           label=f'Synthetic {i+1}', density=True)
        ...
        box_labels = ['Historical'] + [f'Synthetic {i+1}' for i in range(min(5, len(synth_returns_list)))]
        ...
        axes[0, 2].hist(hist_returns[hist_returns < hist_returns.quantile(0.05)], 
                       bins=20, alpha=0.7, label='Historical Left Tail', density=True)
        for i, synth_returns in enumerate(synth_returns_list[:2]):
            left_tail = synth_returns[synth_returns < synth_returns.quantile(0.05)]
            axes[0, 2].hist(left_tail, bins=20, alpha=0.5, 
                           label=f'Synthetic {i+1} Left Tail', density=True)
        ...
        for i, synth_returns in enumerate(synth_returns_list):
            synth_stats = self._calculate_statistics(synth_returns)
            metrics_data.append([f'Synthetic {i+1}', synth_stats['skewness'], synth_stats['kurtosis']])
        ...
        axes[1, 2].bar(x_pos - width/2, metrics_df['Skewness'], width, 
                      label='Skewness', alpha=0.8)
        axes[1, 2].bar(x_pos + width/2, metrics_df['Kurtosis'], width, 
        ...
        if output_dir:
            output_path = Path(output_dir)
            output_path.mkdir(parents=True, exist_ok=True)
            plt.savefig(output_path / f'{ticker}_distribution_analysis.png', 
                       dpi=self.dpi, bbox_inches='tight')
    ...
    def _create_autocorrelation_plots(
        ...
        axes[0, 0].plot(range(1, max_lags + 1), hist_autocorr, 'o-', label='Historical', linewidth=2)
        
        for i, synth_returns in enumerate(synth_returns_list[:3]):
            synth_autocorr = [synth_returns.autocorr(lag=j) for j in range(1, max_lags + 1)]
            axes[0, 0].plot(range(1, max_lags + 1), synth_autocorr, 'o-', 
                           label=f'Synthetic {i+1}', alpha=0.7)
        ...
        axes[0, 1].plot(range(1, max_lags + 1), hist_sq_autocorr, 'o-', label='Historical', linewidth=2)
        
        for i, synth_returns in enumerate(synth_returns_list[:3]):
            synth_sq_autocorr = [(synth_returns**2).autocorr(lag=j) for j in range(1, max_lags + 1)]
            axes[0, 1].plot(range(1, max_lags + 1), synth_sq_autocorr, 'o-', 
                           label=f'Synthetic {i+1}', alpha=0.7)
        ...
        axes[1, 0].plot(range(1, max_lags + 1), hist_abs_autocorr, 'o-', label='Historical', linewidth=2)
        
        for i, synth_returns in enumerate(synth_returns_list[:3]):
            synth_abs_autocorr = [synth_returns.abs().autocorr(lag=j) for j in range(1, max_lags + 1)]
            axes[1, 0].plot(range(1, max_lags + 1), synth_abs_autocorr, 'o-', 
                           label=f'Synthetic {i+1}', alpha=0.7)
        ...
        autocorr_data = []
        autocorr_data.append(['Historical', hist_returns.autocorr(lag=1), 
                             (hist_returns**2).autocorr(lag=1), hist_returns.abs().autocorr(lag=1)])
        
        for i, synth_returns in enumerate(synth_returns_list):
            autocorr_data.append([f'Synthetic {i+1}', synth_returns.autocorr(lag=1),
                                 (synth_returns**2).autocorr(lag=1), synth_returns.abs().autocorr(lag=1)])
        
        autocorr_df = pd.DataFrame(autocorr_data, columns=['Data', 'Returns', 'Squared Returns', 'Absolute Returns'])
        
        x_pos = np.arange(len(autocorr_df))
        width = 0.25
        
        axes[1, 1].bar(x_pos - width, autocorr_df['Returns'], width, 
                      label='Returns', alpha=0.8)
        axes[1, 1].bar(x_pos, autocorr_df['Squared Returns'], width, 
                      label='Squared Returns', alpha=0.8)
        axes[1, 1].bar(x_pos + width, autocorr_df['Absolute Returns'], width, 
                      label='Absolute Returns', alpha=0.8)
        ...
        if output_dir:
            output_path = Path(output_dir)
            output_path.mkdir(parents=True, exist_ok=True)
            plt.savefig(output_path / f'{ticker}_autocorrelation_analysis.png', 
                       dpi=self.dpi, bbox_inches='tight')
    ...
    def _create_price_path_plots(
        ...
        axes[0, 0].plot(historical_data.index, historical_data['Close'], 
                       label='Historical', linewidth=2)
        for i, synth_df in enumerate(synthetic_data[:5]):  # Show first 5 paths
            axes[0, 0].plot(synth_df.index, synth_df['Close'], 
                           label=f'Synthetic {i+1}', alpha=0.7)
        ...
        axes[1, 0].plot(historical_data.index, hist_drawdown, 
                       label='Historical', linewidth=2)
        ...
        for i, synth_df in enumerate(synthetic_data[:3]):
            ...
            axes[1, 0].plot(synth_df.index, synth_drawdown, 
                           label=f'Synthetic {i+1}', alpha=0.7)
        ...
        axes[1, 1].hist(final_prices, bins=20, alpha=0.7, density=True, 
                       label='Synthetic Final Prices')
        axes[1, 1].axvline(hist_final, color='red', linestyle='--', linewidth=2, 
        ...
        if output_dir:
            output_path = Path(output_dir)
            output_path.mkdir(parents=True, exist_ok=True)
            plt.savefig(output_path / f'{ticker}_price_path_analysis.png', 
                       dpi=self.dpi, bbox_inches='tight')
