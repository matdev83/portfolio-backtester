...
class SyntheticDataValidator:
    ...
    def validate_synthetic_data(
        self, original_data: pd.Series, synthetic_data: pd.Series, asset_name: str = "Unknown"
        ...
        # Fat tail validation
        results["fat_tails"] = self._validate_fat_tails(original_data, synthetic_data, asset_name)
    ...
    def _validate_basic_statistics(
        self, original: pd.Series, synthetic: pd.Series, asset_name: str
    ) -> ValidationResults:
        ...
        try:
            ...
            for stat_name in orig_stats:
                ...
                if not isinstance(orig_val, (int, float)) or not isinstance(
                    synth_val, (int, float)
                ):
                    continue
            ...
            return ValidationResults(
                test_name="basic_statistics", passed=False, details={"error": str(e)}
            )
    ...
    def _validate_distribution_similarity(
        ...
        try:
            ...
            if len(orig_clean) < 10 or len(synth_clean) < 10:
                logger.warning(
                    f"Insufficient data for KS test for {asset_name}. Original: {len(orig_clean)}, Synthetic: {len(synth_clean)}"
                )
                return ValidationResults(
            ...
            logger.debug(
                f"KS Test for {asset_name}: Statistic={ks_statistic:.4f}, P-value={p_value:.4f}, Threshold={threshold:.4f}, Passed={passed}"
            )
            ...
            return ValidationResults(
                test_name="distribution_similarity", passed=False, details={"error": str(e)}
            )
    ...
    def _validate_autocorrelation_structure(
        ...
        try:
            ...
            valid_autocorr_pairs = [
                (o, s)
                for o, s in zip(orig_autocorr, synth_autocorr)
                if not (pd.isna(o) or pd.isna(s))
            ]

            if not valid_autocorr_pairs:
                logger.warning(
                    f"No valid autocorrelation values for {asset_name}. Skipping autocorrelation validation."
                )
                return ValidationResults(
            ...
            logger.debug(
                f"Autocorrelation for {asset_name}: Max Diff={max_diff:.4f}, Avg Diff={avg_diff:.4f}, Tolerance={tolerance:.4f}, Passed={passed}"
            )
            ...
            return ValidationResults(
                test_name="autocorrelation_structure", passed=False, details={"error": str(e)}
            )
    ...
    def _validate_volatility_clustering(
        ...
        try:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    f"Volatility Clustering for {asset_name}: Orig Lag1={first_lag_orig:.4f}, Synth Lag1={first_lag_synth:.4f}, Diff={clustering_diff:.4f}, Passed={passed}"
                )
            ...
            return ValidationResults(
                test_name="volatility_clustering", passed=False, details={"error": str(e)}
            )
    ...
    def _validate_rolling_volatility(
        ...
        try:
            window = self.validation_config.get(
                "rolling_volatility_window", 22
            )  # Default to 22 trading days
            tolerance = self.validation_config.get(
                "rolling_volatility_tolerance", 0.3
            )  # 30% tolerance

            if len(original) < window or len(synthetic) < window:
                logger.warning(
                    f"Insufficient data for rolling volatility validation for {asset_name}. Need at least {window} observations."
                )
                return ValidationResults(
            ...
            if min_len == 0:
                logger.warning(
                    f"No valid rolling standard deviations for {asset_name}. Skipping rolling volatility validation."
                )
                return ValidationResults(
                    test_name="rolling_volatility",
                    passed=False,
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    f"Rolling Volatility for {asset_name}: Relative Diff={relative_diff:.4f}, Tolerance={tolerance:.4f}, Passed={passed}"
                )
            ...
            return ValidationResults(
                test_name="rolling_volatility", passed=False, details={"error": str(e)}
            )
    ...
    def _validate_fat_tails(
        ...
        try:
            ...
            kurtosis_tolerance = self.validation_config.get(
                "kurtosis_tolerance", 1.0
            )  # Absolute difference
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    f"Fat Tails for {asset_name}: Orig Tail Index={orig_tail_index:.2f}, Synth Tail Index={synth_tail_index:.2f}, Diff={tail_diff:.2f}, Passed Tail Index={passed_tail_index}\n  Orig Kurtosis={orig_kurtosis_val:.2f}, Synth Kurtosis={synth_kurtosis_val:.2f}, Passed Kurtosis={passed_kurtosis}, Overall Passed={passed}"
                )
            ...
            return ValidationResults(test_name="fat_tails", passed=False, details={"error": str(e)})
    ...
    def _validate_extreme_values(
        ...
        try:
            ...
            if len(original.dropna()) < 100 or len(synthetic.dropna()) < 100:
                logger.warning(
                    f"Insufficient data for extreme value validation for {asset_name}. Need at least 100 observations."
                )
                return ValidationResults(
            ...
            max_percentile_diff: float = np.max(percentile_diffs)

            passed = max_percentile_diff < tolerance

            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    f"Extreme Values for {asset_name}: Max Percentile Diff={max_percentile_diff:.4f}, Tolerance={tolerance:.4f}, Passed={passed}\n  Original Percentiles: {orig_percentiles}\n  Synthetic Percentiles: {synth_percentiles}"
                )
            ...
            return ValidationResults(
                ...
                details={
                    "original_percentiles": {
                        "1%": float(orig_percentiles[0]),
                        "5%": float(orig_percentiles[1]),
                        "95%": float(orig_percentiles[2]),
                        "99%": float(orig_percentiles[3]),
                    },
            ...
            return ValidationResults(
                test_name="extreme_values", passed=False, details={"error": str(e)}
            )
    ...
    def _calculate_overall_quality(
        ...
        try:
            ...
            if total_tests == 0:
                logger.warning(
                    "No individual validation tests performed. Overall quality cannot be calculated."
                )
                return ValidationResults(
            ...
            logger.debug(
                f"Overall Quality: Score={quality_score:.3f}, Passed Tests={passed_tests}/{total_tests}, Threshold={quality_threshold:.3f}, Overall Passed={passed}"
            )
            ...
            return ValidationResults(
                test_name="overall_quality", passed=False, details={"error": str(e)}
            )
    ...
    def _estimate_tail_index(self, data: pd.Series) -> float:
        ...
        try:
            ...
            if len(abs_data) < 20:
                logger.warning(
                    f"Insufficient data ({len(abs_data)}) for Hill tail index estimation. Returning default 3.0."
                )
            ...
            if k <= 0:
                logger.warning(
                    "Not enough data points for Hill estimator after filtering. Returning default 3.0."
                )
                return 3.0
            ...
            if sorted_data[k] == 0:
                logger.warning(
                    "Value at k-th position is zero in Hill estimator. Returning default 3.0."
                )
                return 3.0
            ...
            return float(np.clip(tail_index, 1.5, 10.0))
    ...
    def generate_validation_report(
        self, results: Dict[str, ValidationResults], asset_name: str
    ) -> str:
        """Generate a comprehensive validation report."""
        report_lines = [f"=== Synthetic Data Validation Report for {asset_name} ===", ""]
        ...
        if overall and overall.details:  # Add check for overall.details
            ...
            report_lines.extend([f"Overall Quality: {status} ({score:.1f}%)", ""])
    ...
    def get_validation_summary(self) -> Dict:
        ...
        # Group by test type
        test_type_stats: Dict[str, Dict[str, Union[int, float]]] = {}
