...
class AssetReplacementManager:
    ...
    def __init__(self, config: Dict):
        ...
        if config.get("stage1_optimization", False):
            ...
            optimized_config["generation_config"] = {
                "max_attempts": config.get(
                    "monte_carlo_max_attempts", 1
                ),  # Configurable attempts (default 1 for speed)
        ...
        if logger.isEnabledFor(logging.INFO):
            logger.info(
                f"AssetReplacementManager initialized with {config.get('replacement_percentage', 0.1):.1%} replacement rate"
            )
    ...
    def _load_full_historical_data(self, asset: str) -> Optional[pd.DataFrame]:
        ...
        if not hasattr(self, "data_source") or not hasattr(self, "global_config"):
            logger.warning(
                "Full data source not configured. Cannot load comprehensive historical data."
            )
            return None
        ...
        try:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(
                    f"Loading full historical data for {asset} from {extended_start} to {current_end}"
                )
    ...
    def select_assets_for_replacement(
        self, universe: List[str], random_seed: Optional[int] = None
    ) -> Set[str]:
        """
        ...
        for asset in assets_to_replace:
            ...
            try:
                ...
                if len(period_data) == 0:
                    if logger.isEnabledFor(logging.WARNING):
                        logger.warning(
                            f"No data found for {asset} in period {start_date} to {end_date}"
                        )
                    continue
                ...
                if full_historical_data is not None:
                    ...
                    if logger.isEnabledFor(logging.DEBUG):
                        logger.debug(
                            f"Using {len(historical_data)} days of full historical data for {asset} statistics"
                        )
                else:
                    ...
                    if len(historical_data) < self.config.get("min_historical_observations", 252):
                        ...
                        if self.config.get("stage1_optimization", False):
                            min_obs_required = min(
                                50, min_obs_required
                            )  # Much lower requirement for stage 1

                        if logger.isEnabledFor(logging.WARNING):
                            logger.warning(
                                f"Insufficient historical data for {asset} ({len(historical_data)} < {min_obs_required}). Using all available data."
                            )
                        ...
                        if len(historical_data) == 0:
                            if logger.isEnabledFor(logging.WARNING):
                                logger.warning(
                                    f"No historical data available for {asset}. Skipping synthetic replacement."
                                )
                            continue
                ...
                synthetic_data = self._generate_synthetic_data_for_period(
                    ...
                    target_dates=pd.DatetimeIndex(period_data.index),
                    asset_name=asset,
                )

                # Replace data for the selected time period with synthetic data
                asset_mask = (modified_data[asset].index >= start_date) & (
                    modified_data[asset].index <= end_date
                )
                if asset_mask.any():
                        ...
                        if col in modified_data[asset].columns:
                            try:
                                ...
                                if pd.api.types.is_integer_dtype(original_dtype):
                                    ...
                                    synthetic_col_converted = synthetic_col_data.round().astype(
                                        original_dtype
                                    )
                                elif pd.api.types.is_float_dtype(original_dtype):
                                    # For float columns, direct conversion
                                    synthetic_col_converted = synthetic_col_data.astype(
                                        original_dtype
                                    )
                                else:
    ...
    def _generate_synthetic_data_for_period(
        ...
        synthetic_returns = self.synthetic_generator.generate_synthetic_returns(
            asset_stats=asset_stats, length=target_length, asset_name=asset_name
        )
            ...
            if col not in synthetic_df.columns:
                if col.lower() in ["volume", "vol"]:
                    ...
                    if len(historical_volume) > 0:
                        ...
                        synthetic_volume = np.random.normal(
                            avg_volume, volume_std, len(synthetic_df)
                        )
                        synthetic_volume = np.maximum(
                            synthetic_volume, avg_volume * 0.1
                        )  # Minimum volume
    ...
    def _returns_to_prices(self, returns: np.ndarray, initial_price: float) -> np.ndarray:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            logger.debug(
                f"Using Numba-jitted returns-to-prices conversion for {len(returns)} periods"
            )

        result = returns_to_prices_fast(returns, initial_price)
    ...
    def _generate_ohlc_from_prices(self, prices: np.ndarray) -> np.ndarray:
        ...
        Uses Numba-jitted fast version for 15-30x speedup.
        """
        # Use Numba-jitted fast version for significant speedup
        ...
        result = generate_ohlc_from_prices_fast(prices, random_seed)
        return np.asarray(result)
    ...
    def get_replacement_statistics(self) -> Dict:
        ...
        avg_replacement_percentage = np.mean(
            [info.replacement_percentage for info in self.replacement_history]
        )

        # Count how often each asset was replaced
        asset_replacement_counts: Dict[str, int] = {}
        ...
        return {
            ...
            "most_replaced_assets": sorted(
                asset_replacement_counts.items(), key=lambda x: x[1], reverse=True
            )[:5],
        }
