...
class SignalBasedTiming(TimingController):
    ...
    def _validate_config(self):
        ...
        if self.scan_frequency not in valid_frequencies:
            raise ValueError(
                f"Invalid scan_frequency '{self.scan_frequency}'. Must be one of {valid_frequencies}"
            )

        # Validate holding periods
        if not isinstance(self.min_holding_period, int) or self.min_holding_period < 1:
            raise ValueError(
                f"min_holding_period must be a positive integer, got {self.min_holding_period}"
            )

        if self.max_holding_period is not None:
            if not isinstance(self.max_holding_period, int) or self.max_holding_period < 1:
                raise ValueError(
                    f"max_holding_period must be a positive integer, got {self.max_holding_period}"
                )

            if self.min_holding_period > self.max_holding_period:
                raise ValueError(
                    f"min_holding_period ({self.min_holding_period}) cannot exceed max_holding_period ({self.max_holding_period})"
                )
    ...
    def get_rebalance_dates(
        ...
        date_range = available_dates[
            (available_dates >= start_date) & (available_dates <= end_date)
        ]
        ...
        if self.scan_frequency == "D":
            ...
            freq_mapping = {"W": "W", "M": "ME"}  # Weekly  # Month end
            ...
            scan_dates_list: list[pd.Timestamp] = []
            for scan_date in base_scan_dates:
                ...
                if len(future_dates) > 0:
                    ...
                    if nearest_date <= end_date:
                        scan_dates_list.append(nearest_date)

            scan_dates = pd.DatetimeIndex(sorted(set(scan_dates_list)))
    ...
    def _is_within_min_holding_period(self, current_date: pd.Timestamp) -> bool:
        ...
        return bool(days_since_last < self.min_holding_period)
    ...
    def _should_force_rebalance(self, current_date: pd.Timestamp) -> bool:
        ...
        return bool(days_held >= self.max_holding_period)
    ...
    def can_exit_position(self, asset: str, current_date: pd.Timestamp) -> bool:
        ...
        return bool(holding_days >= self.min_holding_period)
    ...
    def must_exit_position(self, asset: str, current_date: pd.Timestamp) -> bool:
        ...
        holding_days = self.get_position_holding_days(asset, current_date)
        return bool(holding_days >= self.max_holding_period)
