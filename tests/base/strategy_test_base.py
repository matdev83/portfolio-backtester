...
class BaseStrategyTest(unittest.TestCase):
    ...
    def generate_test_data(
        ...
        if pattern == "basic":
            return MarketDataFixture.create_basic_data(
                tickers=tuple(tickers), start_date=start_date, end_date=end_date, seed=seed
            )
    ...
    def assert_strategy_initialization(self, strategy, config: Dict[str, Any]):
        ...
        for method in required_methods:
            self.assertTrue(hasattr(strategy, method), f"Strategy should have {method} method")
            self.assertTrue(
                callable(getattr(strategy, method)), f"Strategy {method} should be callable"
            )

        # Check that strategy configuration is stored
        self.assertTrue(
            hasattr(strategy, "strategy_config"), "Strategy should have strategy_config attribute"
        )
    ...
    def assert_signal_properties(
        ...
        if trade_longs and not trade_shorts:
            min_signal = signals.min().min()
            self.assertGreaterEqual(
                min_signal, -tolerance, f"Found negative signal {min_signal} in long-only strategy"
            )
...
class BaseMomentumStrategyTest(BaseStrategyTest):
    ...
    def calculate_momentum_score(
        self, price_data: pd.DataFrame, lookback_months: int = 3, skip_months: int = 1
    ) -> pd.Series:
        """
    ...
    def assert_momentum_ranking(
        ...
        momentum_scores = self.calculate_momentum_score(price_data, lookback_months, skip_months)
    ...
    def assert_momentum_concentration(
        self, signals: pd.DataFrame, expected_concentration: float = 0.5, tolerance: float = 0.1
    ):
        """
