...
class BaseIntegrationTest(unittest.TestCase):
    ...
    def create_full_backtest_config(
        ...
        **kwargs
        ...
        config = {
            "strategy": {
                "name": strategy_name,
                "params": self.strategy_config["strategy_params"]
            },
    ...
    def assert_backtest_results_valid(
        ...
        self.assertGreaterEqual(len(returns), min_periods, f"Should have at least {min_periods} return periods")
        
        # Validate positions data
        positions = results["positions"]
        self.assertIsInstance(positions, pd.DataFrame, "Positions should be a DataFrame")
        self.assertGreaterEqual(len(positions), min_periods, f"Should have at least {min_periods} position periods")
    ...
    def assert_data_pipeline_integrity(
        ...
        self.assertGreaterEqual(processed_start, input_start, "Processed data start should be >= input start")
    ...
    def assert_data_quality(self, data: pd.DataFrame, max_nan_ratio: float = 0.1):
        ...
        self.assertLessEqual(
            nan_ratio,
            max_nan_ratio,
            f"Too many NaN values: {nan_ratio:.2%} > {max_nan_ratio:.2%}"
        )
    ...
    def test_integration_smoke(self):
        ...
        """
        self.skipTest("Integration smoke test should be implemented in concrete integration classes")
    ...
    def test_end_to_end_workflow_smoke(self):
        ...
        """
        self.skipTest("End-to-end workflow test should be implemented in concrete integration classes")