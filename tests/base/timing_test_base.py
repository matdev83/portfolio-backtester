...
class BaseTimingTest(unittest.TestCase):
    ...
    def assert_valid_rebalance_dates(
        ...
        self.assertEqual(rebalance_dates, sorted_dates, "Rebalance dates should be sorted")
        ...
        self.assertLessEqual(
            last_date, end_date, f"Last rebalance date {last_date} should be <= end date {end_date}"
        )
    ...
    def assert_timing_controller_initialization(
        self, timing_controller, expected_config: Dict[str, Any]
    ):
        """
        ...
        for method in required_methods:
            self.assertTrue(
                hasattr(timing_controller, method), f"Timing controller should have {method} method"
            )
            self.assertTrue(
    ...
    def assert_migration_compatibility(
        self, old_config: Dict[str, Any], new_config: Dict[str, Any], migration_function: callable
    ):
        """
        ...
        for key, expected_value in new_config.items():
            ...
            self.assertEqual(
                migrated_config[key], expected_value, f"Migrated {key} should match expected value"
            )

        # Check that migration is idempotent
        double_migrated = migration_function(migrated_config)
        self.assertEqual(migrated_config, double_migrated, "Migration should be idempotent")
    ...
    def create_migration_test_scenarios(self) -> List[Tuple[Dict[str, Any], Dict[str, Any]]]:
        ...
        new_monthly = {
            ...
            "strategy_params": {"lookback_months": 3},
        }
        scenarios.append((old_monthly, new_monthly))

        # Daily UVXY strategy migration
        old_daily = {"rebalance_frequency": "daily", "strategy_type": "uvxy_rsi", "rsi_period": 2}
        new_daily = {
            "timing_type": "time_based",
    ...
    def run_parameterized_migration_test(
        self, migration_scenarios: List[Tuple[Dict, Dict]], migration_func: callable
    ):
        """
        ...
        for i, (old_config, expected_new_config) in enumerate(migration_scenarios):
            with self.subTest(scenario=i):
                self.assert_migration_compatibility(old_config, expected_new_config, migration_func)
        ...
        for rebalance_date in rebalance_dates[:3]:  # Test first 3 dates
            with self.subTest(date=rebalance_date):
                ...
                should_rebalance = timing_controller.should_rebalance(
                    current_date=rebalance_date, last_rebalance_date=None
                )
                self.assertTrue(should_rebalance, f"Should rebalance on {rebalance_date}")

                # Generate signals
                signals = strategy.generate_signals(
                    all_historical_data=market_data,
                    benchmark_historical_data=market_data.iloc[
                        :, :1
