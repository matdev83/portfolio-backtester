...
class TestIntramonthStrategyIntegration:
    ...
    def wfo_windows(self):
        ...
        windows = [
            ...
            WFOWindow(
                ...
                test_start=pd.Timestamp(2024, 1, 1),
                test_end=pd.Timestamp(2024, 6, 30),
                evaluation_frequency="D",
            ),
            # Window 3: Train on 2024 H1, Test on 2024 H2
            WFOWindow(
                train_start=pd.Timestamp(2024, 1, 1),
                train_end=pd.Timestamp(2024, 6, 30),
                test_start=pd.Timestamp(2024, 7, 1),
    ...
    def optimization_data(
        self, sample_daily_data, sample_monthly_data, sample_returns_data, wfo_windows
    ):
    ...
    def intramonth_scenario_config(self):
        ...
        return {
            ...
            "universe": {"tickers": ["AAPL", "MSFT", "GOOGL", "AMZN", "TSLA"]},
    ...
    def test_complete_intramonth_optimization_workflow(
        self, intramonth_scenario_config, optimization_data, mock_intramonth_strategy
    ):
        """Test complete optimization workflow with intramonth strategy."""
        # Create evaluator with daily evaluation support
        evaluator = BacktestEvaluator(
            metrics_to_optimize=["sharpe_ratio"], is_multi_objective=False, n_jobs=1
        )
        ...
        with patch.object(backtester, "_get_strategy", return_value=mock_intramonth_strategy):
            ...
            test_parameters = {"entry_day": 1, "exit_day": 15, "max_positions": 3}
            ...
            # Unified evaluator path: successful run is sufficient

    @pytest.mark.skipif(not WFO_ENHANCEMENT_AVAILABLE, reason="WFO enhancement not available")
    def test_trade_duration_accuracy_integration(
        self, intramonth_scenario_config, optimization_data, mock_intramonth_strategy
    ):
        ...
        window_evaluator = WindowEvaluator(backtester=mock_backtester)

        # Create a test window (shorter to avoid data issues)
        test_window = WFOWindow(
            train_start=pd.Timestamp(2023, 1, 1),
            train_end=pd.Timestamp(2023, 3, 31),
            test_start=pd.Timestamp(2023, 4, 1),
        ...
        result = window_evaluator.evaluate_window(
            ...
            full_monthly_data=optimization_data.monthly,
    ...
    def test_backward_compatibility_with_monthly_strategies(
        self, monthly_scenario_config, optimization_data, mock_monthly_strategy
    ):
        ...
        with patch.object(backtester, "_get_strategy", return_value=mock_monthly_strategy):
            # Test parameter evaluation
            test_parameters = {"max_positions": 4}
    ...
    @pytest.mark.skipif(not WFO_ENHANCEMENT_AVAILABLE, reason="WFO enhancement not available")
    def test_daily_vs_monthly_evaluation_comparison(
        self, intramonth_scenario_config, optimization_data, mock_intramonth_strategy
    ):
        ...
        with patch.object(backtester, "_get_strategy", return_value=mock_intramonth_strategy):
            ...
            assert daily_result.metrics is not None
            assert monthly_result.metrics is not None

            # Unified evaluator path: successful runs are sufficient
    ...
    def test_multiple_intramonth_strategies_simultaneously(self, optimization_data):
        ...
        for config in strategies_configs:
            ...
            with patch.object(backtester, "_get_strategy", return_value=mock_strategy):
                result = evaluator.evaluate_parameters(
                    parameters=cast(dict[str, Any], config["strategy_params"]),
        ...
        for name, result in results:
            ...
            ), f"Strategy {name} failed to produce valid result"
    ...
    @pytest.mark.skipif(not WFO_ENHANCEMENT_AVAILABLE, reason="WFO enhancement not available")
    def test_position_management_across_window_boundaries(
        self, intramonth_scenario_config, optimization_data, mock_intramonth_strategy
    ):
        ...
        windows = [
            WFOWindow(
                ...
                train_end=pd.Timestamp(2023, 3, 31),
                test_start=pd.Timestamp(2023, 4, 1),
                test_end=pd.Timestamp(2023, 5, 31),
                evaluation_frequency="D",
                strategy_name="SeasonalSignalStrategy",
            ),
            WFOWindow(
                train_start=pd.Timestamp(2023, 2, 1),
                train_end=pd.Timestamp(2023, 4, 30),
                test_start=pd.Timestamp(2023, 5, 1),
                test_end=pd.Timestamp(2023, 6, 30),
                evaluation_frequency="D",
                strategy_name="SeasonalSignalStrategy",
            ),
    ...
    def test_optimization_results_accuracy_validation(
        self, intramonth_scenario_config, optimization_data, mock_intramonth_strategy
    ):
        """Validate that optimization results are mathematically consistent."""
        evaluator = BacktestEvaluator(
            metrics_to_optimize=["sharpe_ratio", "total_return"], is_multi_objective=True, n_jobs=1
        )
    ...
    def test_error_handling_and_recovery(self, intramonth_scenario_config, optimization_data):
        ...
        with patch.object(backtester, "_get_strategy", return_value=failing_strategy):
            ...
            if isinstance(result.objective_value, list):
                assert all(v <= 0 for v in result.objective_value)
            else:
                assert (
                    result.objective_value <= 0
                ), "Failed strategy should have poor objective value"
    ...
    def test_performance_improvements_validation(
        self, intramonth_scenario_config, optimization_data, mock_intramonth_strategy
    ):
        ...
        with patch.object(backtester, "_get_strategy", return_value=mock_intramonth_strategy):
            ...
            if isinstance(result1.objective_value, list) and isinstance(
                result2.objective_value, list
            ):
                assert len(result1.objective_value) == len(result2.objective_value)
                assert all(
                    abs(a - b) < 1e-10
                    for a, b in zip(result1.objective_value, result2.objective_value)
                )
            else:
                assert (
                    abs(cast(float, result1.objective_value) - cast(float, result2.objective_value))
                    < 1e-10
    ...
    @pytest.mark.skipif(not WFO_ENHANCEMENT_AVAILABLE, reason="WFO enhancement not available")
    def test_intramonth_strategy_with_enhanced_wfo(
        self, intramonth_scenario_config, optimization_data
    ):
        """Test SeasonalSignalStrategy with enhanced WFO system."""
        # Ensure configuration is correct and strategy name matches

        # Frequency detection is no longer exposed; ensure config is correct and runs
    ...
    def test_exit_timing_accuracy(self, intramonth_scenario_config, optimization_data):
        ...
        entry_day = intramonth_scenario_config["strategy_params"][
            "SeasonalSignalStrategy.entry_day"
        ]
        hold_days = 10  # Hardcoded for this test

        # Test entry date calculation for different months
        test_months = [pd.Timestamp(2023, 1, 1), pd.Timestamp(2023, 4, 1), pd.Timestamp(2023, 7, 1)]
        ...
        for test_month in test_months:
            ...
            assert (
                7 <= duration_days <= 20
            ), f"Duration {duration_days} should be reasonable for {hold_days} business days"
