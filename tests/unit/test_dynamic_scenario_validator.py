...
class TestDynamicScenarioValidator:
    ...
    def test_portfolio_strategy_validation(self):
        ...
        try:
            ...
            portfolio_errors = [
                e for e in errors if "universe" in e.message or "momentum" in e.message
            ]
            assert (
                len(portfolio_errors) >= 1
            ), f"Should detect portfolio strategy issues, got errors: {[e.message for e in errors]}"
    ...
    def test_signal_strategy_validation(self):
        ...
        config = {
            ...
            "universe_config": {"type": "named", "universe_name": "nasdaq_top20"},
            "strategy_params": {
                "ema_crossover.fast_ema_days": 50,  # Should be less than slow
                "ema_crossover.slow_ema_days": 20,  # Should be greater than fast
                "ema_crossover.timing_config": {"mode": "invalid_mode"},  # Should trigger error
            },
        }
        ...
        try:
            ...
            assert (
                len(signal_errors) >= 1
            ), f"Should detect signal strategy issues, got errors: {[e.message for e in errors]}"

        finally:
            temp_file.unlink()

    def test_diagnostic_strategy_validation(self):
        """Diagnostics are no longer a strategy type; default validation applies uniformly."""
        config = {
        ...
        try:
            ...
            # Uniform validation now; just ensure no crash and windows advisories may appear
    ...
    def test_universal_validation_rules(self):
        ...
        try:
            ...
            assert (
                len(universal_errors) >= 3
            ), f"Should detect multiple universal validation issues, got: {[e.message for e in universal_errors]}"
    ...
    def test_optimization_direction_validation(self):
        ...
        config = {
            ...
            "optimization_targets": [
                {"name": "Sharpe", "direction": "minimize"},  # Should suggest maximize
                {"name": "MaxDrawdown", "direction": "maximize"},  # Should suggest minimize
            ],
            "strategy_params": {"momentum.lookback_months": 11},
        }
        ...
        try:
            ...
            assert (
                len(direction_errors) >= 2
            ), f"Should detect optimization direction issues, got: {[e.message for e in direction_errors]}"
