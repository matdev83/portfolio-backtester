...
class TestHybridDataSource(unittest.TestCase):
    ...
    def test_normalize_data_format_yfinance(self):
        ...
        normalized = self.data_source._normalize_data_format(
            mock_data, "yfinance", ["AAPL", "GOOGL"]
        )
    ...
    def test_fetch_from_source_success(self):
        ...
        data, successful, failed = ds._fetch_from_source(
            "stooq", ["AAPL", "GOOGL"], "2023-01-01", "2023-01-03"
        )

        mock_stooq_instance.get_data.assert_called_once_with(
            ["AAPL", "GOOGL"], "2023-01-01", "2023-01-03"
        )
    ...
    def test_fetch_from_source_partial_failure(self):
        ...
        mixed_data.columns = pd.MultiIndex.from_tuples(
            mixed_data.columns, names=["Ticker", "Field"]
        )
    ...
    def test_get_data_mixed_sources(self):
        ...
        mock_yfinance_instance.get_data.assert_called_once_with(
            ["GOOGL"], "2023-01-01", "2023-01-03"
        )
    ...
    def test_hybrid_data_source_basic_sanity_check(self):
        ...
        try:
            ...
            self.assertNotIn(
                "AAPL", failure_report["total_failures"], "AAPL should not fail on both sources"
            )
...
class TestHybridDataSourceValidation(unittest.TestCase):
    ...
    def test_hybrid_data_source_validation_integration(self):
        ...
        mock_data = pd.DataFrame(
            {("AAPL", "Close"): [100, 101, 102], ("AAPL", "Volume"): [1000, 1100, 1200]}
        )
