...
class TestPositionTracker:
    ...
    def test_multiple_positions(self):
        ...
        signals = pd.DataFrame({"TLT": [1.0], "SPY": [-0.5], "GLD": [0.0]}, index=[date])
    ...
    def test_price_based_pnl_calculation(self):
        ...
        # Create price data
        dates = pd.DatetimeIndex(["2025-01-01", "2025-01-10"])
        prices = pd.DataFrame({"TLT": [100.0, 105.0]}, index=dates)  # 5% price increase
    ...
    def test_multiindex_price_data(self):
        ...
        prices = pd.DataFrame({("TLT", "Close"): [100.0, 105.0]}, index=dates, columns=columns)
