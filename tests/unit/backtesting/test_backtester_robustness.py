...
def corrupted_data():
    ...
    return {"monthly": prices.resample("ME").last().ffill(), "daily": prices, "rets": rets}
...
def test_backtest_with_corrupted_prices(robust_backtester, corrupted_data):
    ...
    with (
        patch.object(robust_backtester, "_get_strategy") as mock_get_strat,
        ...
        mock_sigs.return_value = pd.DataFrame(
            0.5, index=corrupted_data["daily"].index, columns=["A", "B"]
        )
...
def test_backtest_with_misaligned_dates(robust_backtester):
    ...
    monthly = pd.DataFrame(
        100.0, index=pd.date_range("2022-01-01", periods=10, freq="ME"), columns=["A", "SPY"]
    )
    ...
    with (
        patch.object(robust_backtester, "_get_strategy") as mock_get,
        patch(
            "portfolio_backtester.backtesting.strategy_backtester.calculate_portfolio_returns"
        ) as mock_calc,
    ):
...
def test_universe_collection_failure(robust_backtester, corrupted_data):
    ...
    with (
        patch.object(robust_backtester, "_get_strategy") as mock_get_strat,
        patch(
            "portfolio_backtester.interfaces.ticker_collector.TickerCollectorFactory.create_collector"
        ) as mock_factory,
    ):
