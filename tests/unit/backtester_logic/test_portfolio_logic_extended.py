...
def sample_data():
    ...
    price_data = pd.DataFrame(index=dates, columns=pd.MultiIndex.from_product([["A"], ["Close"]], names=["Ticker", "Field"]))
...
def test_calculate_portfolio_returns_with_trade_tracking(sample_data):
    sized_signals, price_data, rets_daily = sample_data
    scenario_config = {"timing_config": {"rebalance_frequency": "D"}, "allocation_mode": "fixed_capital"}
    global_config = {
    ...
    returns, tracker = calculate_portfolio_returns(
        sized_signals, scenario_config, price_data, rets_daily, ["A"], global_config, track_trades=True
    )
