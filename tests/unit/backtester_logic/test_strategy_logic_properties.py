...
@st.composite
def ohlc_data_frames(draw, min_rows=20, max_rows=60, min_assets=2, max_assets=4):
    """
    Generate OHLCV data frames for testing.
    Reduced default sizes for faster test execution.
    """
    ...
    for ticker in all_tickers:
        ...
        # Generate OHLCV data
        opens = prices * draw(
            hnp.arrays(dtype=float, shape=rows, elements=st.floats(min_value=0.98, max_value=1.02))
        )
        highs = np.maximum(
            prices
            * draw(
                hnp.arrays(
                    dtype=float, shape=rows, elements=st.floats(min_value=1.0, max_value=1.05)
                )
            ),
            opens,
        )
        lows = np.minimum(
            ...
            * draw(
                hnp.arrays(
                    dtype=float, shape=rows, elements=st.floats(min_value=0.95, max_value=1.0)
                )
            ),
            opens,
        )
        ...
        volumes = draw(
            hnp.arrays(
                dtype=float, shape=rows, elements=st.floats(min_value=1000, max_value=1000000)
            )
        )
    ...
    for ticker, df in data_dict.items():
        ...
        df_copy.columns = pd.MultiIndex.from_product(
            [[ticker], df_copy.columns], names=["Ticker", "Field"]
        )
        dfs.append(df_copy)
    ...
    if dfs:
        ...
        ohlc_data = pd.DataFrame(
            columns=pd.MultiIndex.from_product(
                [["DUMMY"], ["Open", "High", "Low", "Close", "Volume"]], names=["Ticker", "Field"]
            )
        )
...
def scenario_configs(draw):
    ...
    wfo_start_date = draw(st.one_of(st.none(), st.just(f"{start_year}-01-01")))

    wfo_end_date = draw(st.one_of(st.none(), st.just(f"{end_year}-12-31")))
...
def mock_strategies(draw):
    ...
    def mock_generate_signals(
        all_historical_data, benchmark_historical_data, current_date, **kwargs
    ):
...
def test_size_positions_preserves_signals(ohlc_data_assets, data):
    ...
    with patch(
        "portfolio_backtester.backtester_logic.strategy_logic.size_positions", autospec=True
    ) as mock_size:
