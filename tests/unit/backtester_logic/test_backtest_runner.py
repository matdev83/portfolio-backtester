...
def test_run_scenario_success(runner, mock_dependencies):
    ...
    with patch("portfolio_backtester.backtester_logic.backtest_runner.prepare_scenario_data") as mock_prep, \
         ...
         patch("portfolio_backtester.backtester_logic.backtest_runner.calculate_portfolio_returns") as mock_calc:
...
def test_run_backtest_mode(runner, mock_dependencies):
    ...
    with patch("portfolio_backtester.backtesting.strategy_backtester.StrategyBacktester") as MockBacktester:
        ...
        result = runner.run_backtest_mode(
            scenario_config,
            monthly_data,
            daily_data,
            rets_full
...
def test_run_backtest_mode_with_optuna(runner, mock_dependencies):
    ...
    with patch("portfolio_backtester.backtesting.strategy_backtester.StrategyBacktester") as MockBacktester, \
        ...
        mock_instance = MockBacktester.return_value
        mock_instance.backtest_strategy.return_value = MagicMock(returns=pd.Series(), trade_stats={})
