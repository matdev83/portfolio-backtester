...
class TestPerformanceComparison(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.mock_returns = pd.DataFrame(
            np.random.randn(len(self.dates), 50),
            index=self.dates,
    ...
    def setup_method(self, method):
        ...
        self.historical_data = PerformanceOptimizedFixtures.generate_cached_ohlcv_data(
            start_date="2020-01-01", end_date="2023-12-31", num_assets=100, freq="D"
        )

        self.benchmark_data = PerformanceOptimizedFixtures.generate_cached_benchmark_data(
            start_date="2020-01-01", end_date="2023-12-31", freq="D"
        )
    ...
    def test_timing_controller_initialization_performance(self):
        ...
        print(
            f"Timing controller initialization times: {[f'{t:.3f}s' for t in initialization_times]}"
        )
    ...
    def test_rebalance_date_generation_performance(self):
        ...
        for config, expected_count in configs_and_expected_counts:
            ...
            ), f"Rebalance date generation too slow for {config}: {end_time - start_time:.3f}s"

            # Verify reasonable number of dates
            assert len(rebalance_dates) > 0, f"No rebalance dates generated for {config}"
            assert (
                abs(len(rebalance_dates) - expected_count) < expected_count * 0.5
    ...
    def test_signal_generation_performance_comparison(self):
        ...
        strategy = SimpleMomentumPortfolioStrategy(momentum_config)
        ...
        for current_date in test_dates:
            ...
            strategy.generate_signals(
                ...
                benchmark_historical_data=self.benchmark_data[
                    self.benchmark_data.index <= current_date
                ],
    ...
    def test_timing_state_update_performance(self):
        ...
        controller = SignalBasedTiming(
            {"scan_frequency": "D", "min_holding_period": 1, "max_holding_period": 5}
        )
        ...
        assert (
            end_time - start_time < 0.5
        ), f"Timing state updates too slow: {end_time - start_time:.3f}s"
    ...
    def test_concurrent_timing_controller_performance(self):
        ...
        ), f"Concurrent timing operations too slow: {end_time - start_time:.3f}s"

        print(
            f"Concurrent timing operations (4 controllers, 52 weeks): {end_time - start_time:.3f}s"
        )
...
class TestMemoryUsage:
    ...
    def test_timing_state_memory_efficiency(self):
        ...
        controller = SignalBasedTiming(
            {"scan_frequency": "D", "min_holding_period": 1, "max_holding_period": None}
        )
        ...
        for i, date in enumerate(dates):
            ...
            if i % 10 == 0:  # Change positions every 10 days
                ...
                if current_positions:
                    ...
                    positions_to_close = np.random.choice(
                        list(current_positions), size=min(25, len(current_positions)), replace=False
                    )
                ...
                new_positions = np.random.choice(
                    available_assets, size=min(25, len(available_assets)), replace=False
                )
                ...
                if len(active_weights) > 0:
                    prices = pd.Series(
                        np.random.uniform(50, 200, len(active_weights)), index=active_weights.index
                    )
        ...
        assert len(state.position_entry_dates) >= len(
            current_positions
...
class TestScalabilityLimits:
    ...
    def test_large_universe_performance(self):
        ...
        controller = SignalBasedTiming(
            {"scan_frequency": "D", "min_holding_period": 1, "max_holding_period": 10}
        )
        ...
        ), f"Large universe update too slow: {end_time - start_time:.3f}s"
    ...
    def test_extreme_date_ranges(self):
        ...
        assert (
            end_time - start_time < 2.0
        ), f"Extreme date range too slow: {end_time - start_time:.3f}s"
