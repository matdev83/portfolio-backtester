...
class TestTimeBasedTiming:
    ...
    def test_get_rebalance_dates_monthly(self):
        ...
        result = controller.get_rebalance_dates(start_date, end_date, available_dates, strategy_context)
    ...
    def test_get_rebalance_dates_quarterly(self):
        ...
        assert len(result) >= 3  # At least Q1, Q2, Q3 (Q4 might be beyond range due to weekend adjustment)
        assert result[0] == pd.Timestamp('2023-03-31')  # Q1 end
        assert result[1] == pd.Timestamp('2023-06-30')  # Q2 end
        # Q3 end (Sep 30 is Saturday, so next business day is Oct 2)
        assert result[2] == pd.Timestamp('2023-10-02')
    ...
    def test_get_rebalance_dates_daily(self):
        ...
        expected_days = 5  # Mon-Fri
        assert len(result) == expected_days
        assert result[0] == pd.Timestamp('2023-01-02')  # First Monday
    ...
    def test_get_rebalance_dates_weekend_adjustment(self):
        ...
        end_date = pd.Timestamp('2023-05-05')  # Extend end date to allow weekend adjustment
        available_dates = pd.bdate_range('2023-04-01', '2023-05-05')  # Include next week
        strategy_context = Mock()
        
        result = controller.get_rebalance_dates(start_date, end_date, available_dates, strategy_context)
        
        # Should get next business day (May 1, 2023 is a Monday)
        assert len(result) == 1
        assert result[0] == pd.Timestamp('2023-05-01')
    ...
    def test_get_rebalance_dates_weekly(self):
        ...
        # Should get weekly dates (Sundays, adjusted to next business day)
        assert len(result) >= 3  # At least 3-4 weeks
        # First week ends on Sunday Jan 8, next business day is Monday Jan 9
    ...
    def test_get_rebalance_dates_annual(self):
        ...
        # Should get year-end dates
        assert len(result) >= 2  # 2022, 2023 (2024 might be beyond range)
        # Dec 31, 2022 is Saturday, so next business day is Jan 2, 2023
        assert result[0] == pd.Timestamp('2023-01-02')
    ...
    def test_get_rebalance_dates_single_available_date(self):
        ...
        # Should return the single date if it's after a scheduled date
        assert len(result) <= 1
        if len(result) == 1:
    ...
    def test_case_insensitive_frequency(self):
        ...
        result_lower = controller_lower.get_rebalance_dates(start_date, end_date, available_dates, strategy_context)
        result_upper = controller_upper.get_rebalance_dates(start_date, end_date, available_dates, strategy_context)
