...
def test_is_all_zero():
    ...
    assert (
        _is_all_zero(pd.Series([], dtype=float)) is False
    )  # Empty is not "all zero" returns, handled separately
...
def test_calculate_max_dd_recovery_time_never_recovers():
    ...
    assert (
        calculate_max_dd_recovery_time(rets) == 0
    )  # Or whatever behavior is expected for unrecovered DD
...
def test_calculate_metrics_full_run(random_returns, benchmark_returns):
    metrics = calculate_metrics(random_returns, benchmark_returns, "SPY", num_trials=100)
...
def test_deflated_sharpe_ratio_edge_cases():
    ...
    const_rets = pd.Series([0.01] * 150, index=dates_const)
...
def test_deflated_sharpe_requires_multiple_trials_hypothesis_adjustment():
    idx = pd.date_range("2020-01-01", periods=200, freq="B")
    rng = np.random.default_rng(7)
    rets_s = pd.Series(rng.normal(0.001, 0.012, len(idx)), index=idx)
    bench_s = pd.Series(rng.normal(0.0005, 0.009, len(idx)), index=idx)

    m_one = calculate_metrics(rets_s, bench_s, "SPY", num_trials=1)
    assert np.isnan(m_one["Deflated Sharpe"])

    m_many = calculate_metrics(rets_s, bench_s, "SPY", num_trials=200)
    assert not np.isnan(m_many["Deflated Sharpe"])
    assert np.isfinite(float(m_many["Deflated Sharpe"]))
    assert abs(float(m_many["Deflated Sharpe"]) - 1.0) > 1e-15
