...
from src.portfolio_backtester.utils import _resolve_strategy, _df_to_float32_array


class TestResolveStrategyPolymorphic:
    """Test _resolve_strategy function with polymorphic implementation."""

    @patch('src.portfolio_backtester.interfaces.strategy_resolver_interface.PolymorphicStrategyResolver.enumerate_strategies_with_params')
...
class TestDfToFloat32ArrayPolymorphic:
    ...
    def test_convert_invalid_input_raises_error(self):
        ...
        with pytest.raises(TypeError, match="Input must be a pandas DataFrame"):
            _df_to_float32_array("not_a_dataframe")
...
class TestPolymorphicEquivalence:
    ...
    def test_array_conversion_equivalence(self):
        ...
        assert matrix.shape == (2, 2)
        assert tickers == ["AAPL", "GOOGL"]
        assert np.allclose(matrix, expected_values.values)
