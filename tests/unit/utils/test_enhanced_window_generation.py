...
from src.portfolio_backtester.utils import generate_enhanced_wfo_windows, _determine_evaluation_frequency
...
class TestEnhancedWindowGeneration:
    ...
    @patch('src.portfolio_backtester.utils.generate_randomized_wfo_windows')
    def test_generate_enhanced_wfo_windows_with_wfo_window_available(self, mock_generate_base):
        ...
        mock_generate_base.return_value = [
            ...
            (pd.Timestamp('2024-02-01'), pd.Timestamp('2025-01-31'), 
             pd.Timestamp('2025-02-01'), pd.Timestamp('2025-02-28'))
        ]
        
        # Test data
        monthly_data_index = pd.date_range('2024-01-01', '2025-12-31', freq='M')
        scenario_config = {
            'strategy_class': 'IntramonthSeasonalStrategy',
            'name': 'test_strategy'
        }
    ...
    def test_generate_enhanced_wfo_windows_with_random_state(self, mock_generate_base):
        ...
        # Test data
        monthly_data_index = pd.date_range('2024-01-01', '2025-12-31', freq='M')
        scenario_config = {'strategy_class': 'MomentumStrategy'}
        global_config = {}
        random_state = 42
    ...
    def test_generate_enhanced_wfo_windows_preserves_window_boundaries(self, mock_generate_base):
        ...
        original_windows = [
            (pd.Timestamp('2024-01-15'), pd.Timestamp('2024-11-30'), 
             pd.Timestamp('2024-12-01'), pd.Timestamp('2024-12-31')),
            (pd.Timestamp('2024-03-01'), pd.Timestamp('2025-01-31'), 
             pd.Timestamp('2025-02-01'), pd.Timestamp('2025-02-28'))
        ]
