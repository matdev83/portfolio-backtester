...
from tests.strategies.optimization_strategies import parameter_spaces, optimization_configs
...
def test_genetic_generator_evolution(param_space, population_size):
    ...
    assert any(
        all(ind[k] == best_individual[k] for k in ind.keys()) for ind in next_generation
...
def test_genetic_performance_optimizer_creation(config):
    ...
    optimizer = GeneticPerformanceOptimizer(
        enable_deduplication=True, enable_parallel_execution=True, n_jobs=4
    )
    ...
    assert optimizer._deduplicator is None
    assert optimizer._parallel_runner is None
...
def test_genetic_generator_termination(param_space):
    ...
    for _ in range(3):
        ...
        for i, individual in enumerate(population):
            result = EvaluationResult(
                objective_value=float(i), metrics={"fitness": float(i)}, window_results=[]
            )
            results.append(result)
