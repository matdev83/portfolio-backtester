...
class TestOptimizationConfigValidation:
    ...
    def test_valid_optimization_config(self):
        ...
        valid_config = {"metrics_to_optimize": ["sharpe_ratio"], "max_evaluations": 100}
    ...
    def test_invalid_max_evaluations(self):
        ...
        with pytest.raises(ParameterGeneratorError):
            validate_optimization_config(
                {"metrics_to_optimize": ["sharpe_ratio"], "max_evaluations": -1}
            )
...
class TestParameterGeneratorFactory:
    ...
    def test_create_genetic_generator(self):
        ...
        try:
            ...
            generator = create_parameter_generator("genetic", random_state=rng)
            assert generator is not None
        except ImportError:
            # Genetic algorithm dependencies not available, skip test
            pytest.skip("Genetic algorithm dependencies not available")
