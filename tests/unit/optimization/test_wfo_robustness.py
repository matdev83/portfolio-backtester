...
class TestWFORobustness:
    ...
    def wfo_robustness_config(self):
        ...
        return {
            ...
            "train_window_randomization": {"min_offset": 3, "max_offset": 14},
            "test_window_randomization": {"min_offset": 3, "max_offset": 14},
            "start_date_randomization": {"min_offset": 0, "max_offset": 12},
            "stability_metrics": {
                "enable": True,
                "worst_percentile": 10,
    ...
    def global_config(self, wfo_robustness_config):
        ...
        return {"wfo_robustness_config": wfo_robustness_config, "benchmark": "SPY"}
    ...
    def test_generate_randomized_wfo_windows_basic(
        self, monthly_data_index, scenario_config, global_config
    ):
        ...
        windows = generate_randomized_wfo_windows(
            monthly_data_index, scenario_config, config, rng=rng
        )
        ...
        for train_start, train_end, test_start, test_end in windows:
            ...
            if robustness_config["enable_window_randomization"]:
                assert (
                    train_months
                    >= scenario_config["train_window_months"]
                    + robustness_config["train_window_randomization"]["min_offset"]
                )
                assert (
                    train_months
                    <= scenario_config["train_window_months"]
                    + robustness_config["train_window_randomization"]["max_offset"]
                    + 1
                )  # +1 for potential month boundary
                assert (
                    test_months
                    >= scenario_config["test_window_months"]
                    + robustness_config["test_window_randomization"]["min_offset"]
                )
                assert (
                    test_months
                    <= scenario_config["test_window_months"]
                    + robustness_config["test_window_randomization"]["max_offset"]
                    + 1
                )  # +1 for potential month boundary
            else:
                ...
                if scenario_config["walk_forward_type"] == "expanding":
                    ...
                    if prev_train_months is None:
                        ...
                        expected_train_months = (
                            prev_train_months + scenario_config["test_window_months"]
                        )
    ...
    def test_rolling_vs_expanding_windows(self, monthly_data_index, scenario_config, global_config):
        ...
        windows_expanding = generate_randomized_wfo_windows(
            monthly_data_index, scenario_expanding, global_config, rng=rng
        )
        ...
        windows_rolling = generate_randomized_wfo_windows(
            monthly_data_index, scenario_rolling, global_config, rng=rng
        )
        ...
        if len(windows_expanding) > 1:
            ...
            assert all(
                train_sizes_expanding[i] <= train_sizes_expanding[i + 1]
    ...
    def test_calculate_stability_metrics(self, global_config):
        ...
        assert sharpe_worst <= max(
            metric_values_per_objective[0]
    ...
    def test_stability_metrics_all_nan_values(self, global_config):
        ...
        # All NaN values
        metric_values_per_objective = [[np.nan, np.nan, np.nan], [np.nan, np.nan, np.nan]]
    ...
    def test_wfo_robustness_configuration_validation(self, monthly_data_index, scenario_config):
        ...
        windows = generate_randomized_wfo_windows(
            monthly_data_index, scenario_config, minimal_config, rng=rng
        )
        ...
        try:
            windows = generate_randomized_wfo_windows(
                monthly_data_index, scenario_config, invalid_config, rng=rng
            )
    ...
    def test_reproducibility_with_random_seed(
        ...
        windows1 = generate_randomized_wfo_windows(
            monthly_data_index, scenario_config, global_config, rng=rng1
        )

        windows2 = generate_randomized_wfo_windows(
            monthly_data_index, scenario_config, global_config, rng=rng2
        )
    ...
    def test_different_seeds_produce_different_results(
        ...
        assert differences > 0 or len(windows1) != len(
            windows2
    ...
    def test_insufficient_data_handling(self, scenario_config, global_config):
        ...
        windows = generate_randomized_wfo_windows(
            short_index, scenario_config, global_config, rng=rng
        )
