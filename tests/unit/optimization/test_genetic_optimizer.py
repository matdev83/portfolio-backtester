...
class TestGeneticOptimizerWithWalkForward:
    ...
    def backtester_for_ga(self, tmp_path):
        ...
        date_index_monthly = pd.date_range(start='2021-01-01', end='2023-01-01', freq='BME')
        ...
        date_index_daily = pd.date_range(start='2021-01-01', end='2023-01-01', freq='B')
    ...
    def test_ga_optimizer_with_mocked_backtester_evaluation(self, backtester_for_ga):
        ...
        def fitness_func_wrapper_capture(ga_instance, solution, solution_idx):
            evaluated_solutions.append(list(solution))
            # Return a dummy fitness value
            return 1.0
        ...
        # Check that not all evaluated solutions are the same
        assert len(set(map(tuple, evaluated_solutions))) > 1
