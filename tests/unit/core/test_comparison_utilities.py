    ...
    def compare_arrays(
        ...
        if optimized_result.shape != reference_result.shape:
            return ComparisonResult(
                ...
                details={'error': f'Shape mismatch: {optimized_result.shape} vs {reference_result.shape}'}
            )
...
class PerformanceMonitor:
    ...
    def compare_performance(
        ...
        _, opt_time = self.time_function(
            optimized_func, args, kwargs, 'optimized', num_runs
        )
        
        # Time reference function
        _, ref_time = self.time_function(
            reference_func, args, kwargs, 'reference', num_runs
        )
