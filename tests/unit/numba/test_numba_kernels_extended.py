...
def test_detailed_commission_slippage_kernel():
    ...
    T, N = 3, 2
    weights = np.array([
        [0.5, 0.5],  # Day 0: turnover is treated as 0 in this kernel logic (except initialization logic check)
                     # Wait, kernel says: if i == 0: out[i] = 0.0; continue. 
                     # So Day 0 costs are 0.
    ...
    cost_frac, detailed_cost_frac = detailed_commission_slippage_kernel(
        weights, close_prices, portfolio_value, 
        commission_per_share, commission_min, commission_max_pct, slippage_bps,
        price_mask
    )
    
    # Day 0: 0 cost
    assert cost_frac[0] == 0.0
...
def test_trade_tracking_kernel_reinvestment():
    ...
    pvals, cash, pos = trade_tracking_kernel(
        initial_pv, 0, weights, prices, mask, commissions
    )
...
def test_trade_lifecycle_kernel_basic():
    ...
    commissions = np.array([
        [0.0], [1.0], [1.0] # Absolute commission values here?
...
def test_calculate_window_return_numba():
    ...
    signals = np.array([
        [1.0],
        [1.0] # Signal at Day 1 doesn't affect Day 1 return (lagged logic usually applies, but kernel uses signals[day-1])
