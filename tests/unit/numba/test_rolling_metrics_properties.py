...
@given(return_series(min_size=20, max_size=100))
...
def non_degenerate_return_matrices(draw):
    ...
    matrix = draw(
        return_matrices(
            min_rows=30, max_rows=100, min_cols=2, max_cols=5, ensure_nonzero_variance=True
        )
    )
    ...
    for t in range(window - 1, returns_matrix.shape[0]):
        for a in range(returns_matrix.shape[1]):
            ...
            if np.all(window_returns >= target_return) and np.mean(window_returns) > target_return:
                ...
                assert (
                    np.isnan(sortino_ratios[t, a])
                    or sortino_ratios[t, a] > 100
                    or np.isinf(sortino_ratios[t, a])
                )
...
def correlated_returns(draw):
    ...
    for i in range(cols):
        ...
        asset_returns[:, i] = (
            correlation * benchmark_returns + np.sqrt(1 - correlation**2) * idiosyncratic
        )

        # Scale to target volatility
        asset_returns[:, i] *= (
            vol / np.std(asset_returns[:, i]) if np.std(asset_returns[:, i]) > 0 else 1.0
        )
