...
def test_populate_from_kernel_results_raises_without_legacy_opt_in(tracker):
    dates = pd.date_range("2023-01-01", periods=2)
    portfolio_values = pd.Series([100000.0, 101000.0], index=dates)
    positions = pd.DataFrame(0.0, index=dates, columns=["A"])
    ...
    completed_trades = np.zeros(0, dtype=dt)
    with pytest.raises(RuntimeError, match="allow_legacy_close_trade_inference"):
...
def test_update_positions_logic(tracker):
    ...
    with (
        patch.object(tracker.trade_lifecycle_manager, "open_position") as mock_open,
        patch.object(tracker.trade_lifecycle_manager, "close_position") as _mock_close,
        patch.object(
            tracker.portfolio_value_tracker, "update_daily_metrics"
...
def test_close_all_positions(tracker):
    ...
    with (
        patch.object(tracker.trade_lifecycle_manager, "close_all_positions") as mock_close_all,
