...
@pytest.mark.parametrize(
    "strategy_class, config, expected_tunable_params, expected_non_universe_reqs",
    [
        (
            UvxyRsiSignalStrategy,
            {"strategy_params": {"rsi_period": 2, "rsi_threshold": 30.0}},
            {"rsi_period", "rsi_threshold"},
            ["SPY"],
        ),
        (
            EmaRoroSignalStrategy,
            {"fast_ema_days": 10, "slow_ema_days": 20},
            {"fast_ema_days", "slow_ema_days", "leverage", "risk_off_leverage_multiplier"},
            [],
        ),
    ],
)
class TestStrategyInitialization:
    def test_strategy_initialization(
        self, strategy_class, config, expected_tunable_params, expected_non_universe_reqs
    ):
    ...
    def test_non_universe_data_requirements(
        self, strategy_class, config, expected_tunable_params, expected_non_universe_reqs
    ):
        strategy = strategy_class(config)
