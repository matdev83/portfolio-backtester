...
class TestBaseStrategy:
    ...
    def test_trade_direction_initialization(self):
        # Default: True/True
        s1 = TestStrategy({"strategy_params": {}}) 
        assert s1.trade_longs is True
        assert s1.trade_shorts is True

        # Custom
        s2 = TestStrategy({"strategy_params": {"trade_longs": False, "trade_shorts": True}})
        assert s2.trade_longs is False
        assert s2.trade_shorts is True

    def test_invalid_trade_direction_config(self):
        # Both False
        with pytest.raises(TradeDirectionConfigurationError, match="Both trade_longs and trade_shorts are False"):
            TestStrategy({"strategy_params": {"trade_longs": False, "trade_shorts": False}})

        # Invalid Types
        with pytest.raises(TradeDirectionConfigurationError, match="must be boolean"):
            TestStrategy({"strategy_params": {"trade_longs": "yes", "trade_shorts": True}})
    ...
    def test_validate_data_sufficiency(self, strategy):
        # Mock min periods
        with patch.object(TestStrategy, 'get_minimum_required_periods', return_value=12):
            ...
            is_valid, msg = strategy.validate_data_sufficiency(
                data, pd.DataFrame(), current_date
            )
    ...
    def test_filter_universe_by_data_availability(self, strategy):
        ...
        with patch.object(TestStrategy, 'get_minimum_required_periods', return_value=6): # 6 months required
