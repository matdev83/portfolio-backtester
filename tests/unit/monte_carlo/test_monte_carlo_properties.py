...
def test_asset_replacement_manager_selection(config, universe):
    ...
    assert manager.replacement_history[0].replacement_percentage == config.get(
...
def ohlc_data_for_replacement(draw):
    ...
    for asset in assets:
        ...
        changes = draw(
            hnp.arrays(
                dtype=float, shape=total_days, elements=st.floats(min_value=-0.05, max_value=0.05)
            )
        )
    ...
    if total_days < history_len + replacement_len:
        ...
        replacement_start_idx = draw(
            st.integers(min_value=history_len, max_value=total_days - replacement_len)
        )
    ...
    for asset in assets:
        ...
        if asset not in assets_to_replace:
            ...
            pd.testing.assert_frame_equal(
                modified_data[asset].loc[before_mask], original_data[asset].loc[before_mask]
            )

            # Data in period should be different (synthetic)
            period_mask = (original_data[asset].index >= start_date) & (
                original_data[asset].index <= end_date
            )
            if period_mask.any():
...
def test_monte_carlo_simulation_preserves_statistical_properties(returns):
    ...
    # Skip test if returns contain NaN values
    assume(not returns.isna().any())
    monthly_returns = _coerce_returns_to_monthly(returns.dropna())
    # Calculate original statistical properties on the same frequency
    original_mean = monthly_returns.mean()
    original_std = monthly_returns.std()

    # Run Monte Carlo simulation
    simulation_results = run_monte_carlo_simulation(
        strategy_returns=returns, n_simulations=100, n_years=5, initial_capital=1.0
    )
    ...
    assert (path_means.quantile(0.25) < original_mean < path_means.quantile(0.75)) or np.isclose(
        path_means.median(), original_mean, rtol=0.5, atol=0.01
    )
