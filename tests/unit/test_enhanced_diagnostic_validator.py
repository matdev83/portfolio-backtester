...
class TestEnhancedDiagnosticValidator:
    """Test the enhanced validation capabilities for diagnostic configs."""
    
    def test_valid_diagnostic_config(self):
        """Test that a valid diagnostic config passes validation."""
        config_file = Path("config/scenarios/diagnostic/default.yaml")
        errors = validate_scenario_file(config_file, OPTIMIZER_PARAMETER_DEFAULTS)
        
        # Should have no errors
        assert len(errors) == 0, f"Valid config should pass validation, but got errors: {[e.message for e in errors]}"
    ...
    def test_parameter_value_range_validation(self):
        ...
        config = {
            ...
            "strategy_params": {
                "stop_loss_tester.atr_length": 50  # Outside range [5, 30]
            }
        }
    ...
    def test_step_size_validation(self):
        ...
        try:
            ...
            step_errors = [e for e in errors if "step size" in e.message and "larger than" in e.message]
    ...
    def test_window_size_warnings(self):
        ...
        try:
            ...
            window_warnings = [e for e in errors if "window" in e.message and ("too short" in e.message or "may be" in e.message)]
            assert len(window_warnings) >= 2, f"Should detect multiple window warnings, got {len(window_warnings)}"
