...
def temp_project_structure(tmp_path: Path):
    ...
    # Strategy categories
    categories = ["signal", "portfolio", "meta"]
    ...
    bad_py_naming_strategy_dir = src_dir / "portfolio" / "bad_py_naming_strategy"
    ...
    # Corresponding config
    bad_config_dir = scenarios_dir / "portfolio" / "bad_py_naming_strategy"
    ...
    with open(
        empty_config_dir / "default.yaml", "w"
    ) as f:  # Config exists, but no .py files to check naming
...
def test_validate_strategy_configs_all_good(temp_project_structure):
    ...
    assert any(
        "missing_config_strategy' does not have a corresponding default.yaml" in error
        for error in errors
    )
    assert (
        "helper_utils.py' in strategy 'good_signal_strategy' directory does not contain the strategy name"
        in "\n".join(errors)
    )
    assert (
        "some_random_helper.py' in strategy 'bad_py_naming_strategy' directory does not contain the strategy name"
        in "\n".join(errors)
    )
...
def test_validate_strategy_configs_all_good_py_naming(temp_project_structure):
    ...
    error_messages = "\n".join(errors)
    assert (
        "all_good_py_naming_sub_module.py' in strategy 'all_good_py_naming' directory does not contain the strategy name ('all_good_py_naming') in its filename."
...
def test_validate_strategy_configs_empty_strategy_dir(temp_project_structure):
    ...
    error_messages = "\n".join(errors)
    assert (
        "empty_strategy_dir' directory does not contain the strategy name" not in error_messages
    )  # No .py files to check for *this* strategy
    assert "missing_config_strategy' does not have a corresponding default.yaml" in error_messages
    assert len(errors) == 3  # missing_config_strategy, helper_utils.py, some_random_helper.py
