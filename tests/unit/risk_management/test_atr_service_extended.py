...
class TestOptimizedATRService:
    ...
    def test_calculate_atr_naive_current_date_tz_aware_index(self, service, ohlc_data):
        tz_ix = ohlc_data.index.tz_localize("America/New_York")
        data_tz = ohlc_data.copy()
        data_tz.index = tz_ix
        last_naive = pd.Timestamp(tz_ix[-1].year, tz_ix[-1].month, tz_ix[-1].day)
        res = service.calculate_atr(data_tz, last_naive, atr_length=14)
        assert "AAPL" in res.index
        assert not np.isnan(res["AAPL"])
