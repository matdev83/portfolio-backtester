...
def parameter_values(draw, param_space):
    ...
    for param_name, param_config in param_space.items():
        ...
        if param_type == "int":
            params[param_name] = draw(
                st.integers(min_value=param_config["low"], max_value=param_config["high"])
            )
...
def optimization_configs(draw):
    ...
    config = {
        ...
        "timeout_seconds": draw(st.one_of(st.none(), st.integers(min_value=10, max_value=300))),
        "early_stop_patience": draw(st.one_of(st.none(), st.integers(min_value=5, max_value=20))),
    }
...
def ga_worker_contexts(draw):
    ...
    return {
        "run_id": draw(
            st.text(alphabet="abcdefghijklmnopqrstuvwxyz0123456789", min_size=8, max_size=8)
        ),
...
def populations_with_results(draw, param_space=None, population_size=None):
    ...
    for i, params in enumerate(population):
        ...
        result = EvaluationResult(
            objective_value=fitness, metrics={"fitness": fitness}, window_results=[]
        )
