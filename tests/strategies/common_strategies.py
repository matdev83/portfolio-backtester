...
@st.composite
def timestamps(draw, min_year: int = 2000, max_year: int = 2023, size: Optional[int] = None):
    """
    ...
    if allow_missing:
        ...
        values = draw(
            hnp.arrays(
                ...
                shape=size,
                elements=st.floats(
                    min_value=min_price, max_value=max_price, allow_nan=False, allow_infinity=False
                ),
                fill=st.nothing(),
            )
        )
...
def return_matrices(
    ...
    elements = st.floats(
        min_value=min_return, max_value=max_return, allow_nan=False, allow_infinity=False
    )
...
def price_dataframes(
    ...
    if include_ohlcv:
        ...
        cols = pd.MultiIndex.from_product(
            [tickers, ["Open", "High", "Low", "Close", "Volume"]], names=["Ticker", "Field"]
        )
        ...
        for ticker in tickers:
            ...
            for i in range(rows):
                ...
                df.loc[index[i], (ticker, "Volume")] = draw(
...
def weights_and_leverage(
    ...
    leverage = draw(
        st.floats(
            min_value=min_leverage, max_value=max_leverage, allow_nan=False, allow_infinity=False
        )
    )
