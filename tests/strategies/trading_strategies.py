...
@st.composite
def weights_and_prices(
    draw, min_days: int = 10, max_days: int = 100, min_assets: int = 2, max_assets: int = 10
):
    """
...
def trade_tracking_inputs(draw):
    ...
    portfolio_value = draw(
        st.floats(min_value=10000.0, max_value=1000000.0, allow_nan=False, allow_infinity=False)
    )
