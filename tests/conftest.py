...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "integration: marks tests as integration tests (moderate speed)")
    ...
    config.addinivalue_line("markers", "data_sources: marks tests related to data source functionality")
...
def assert_strategy_signals_valid(signals: pd.DataFrame, expected_columns: Optional[List[str]] = None):
    """
...
def assert_performance_metrics_valid(metrics: Dict[str, float], thresholds: Optional[Dict[str, float]] = None):
    """
    ...
    if thresholds:
        if 'min_sharpe_ratio' in thresholds:
            assert metrics['sharpe_ratio'] >= thresholds['min_sharpe_ratio'], \
                f"Sharpe ratio {metrics['sharpe_ratio']} below threshold {thresholds['min_sharpe_ratio']}"
        
        if 'max_drawdown' in thresholds:
            assert abs(metrics['max_drawdown']) <= thresholds['max_drawdown'], \
...
def pytest_sessionfinish(session, exitstatus):
    ...
    if cache_info['lru_cache_hits'] + cache_info['lru_cache_misses'] > 0:
        hit_rate = cache_info['lru_cache_hits'] / (cache_info['lru_cache_hits'] + cache_info['lru_cache_misses'])
