...
class OptimizedDataGenerator:
    ...
    def generate_cached_ohlcv_data(
        ...
        if len(date_range) == 0:
            raise ValueError(
                f"Empty date range generated for {start_date} to {end_date} with freq {freq}"
            )
    ...
    def _generate_price_series(
        length: int, pattern: str, base_price: float, volatility: float
    ) -> np.ndarray:
        """
    ...
    def _create_ohlcv_from_prices(
        prices: np.ndarray, pattern: str, volatility: float
    ) -> np.ndarray:
        """
    ...
    @staticmethod
    def create_standardized_date_range(start: str, end: str, freq: str = "ME") -> pd.DatetimeIndex:
        """
        ...
        try:
            ...
            raise ValueError(
                f"Failed to create date range from {start} to {end} with freq {standardized_freq}: {e}"
            )

    @staticmethod
    def create_business_date_range(
        start: str, end: str, holidays: Optional[List[str]] = None
    ) -> pd.DatetimeIndex:
        """
    ...
    def validate_ohlcv_data_structure(data: pd.DataFrame) -> bool:
        ...
        if list(data.columns.names) != expected_names:
            raise ValueError(
                f"Column names must be {expected_names}, got {list(data.columns.names)}"
            )
    ...
    def validate_date_range_consistency(
        data: pd.DataFrame, expected_start: str, expected_end: str, expected_freq: str
    ) -> bool:
        """
