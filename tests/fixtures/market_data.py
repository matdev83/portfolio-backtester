...
class MarketDataFixture:
    ...
    def create_volatile_data(
        ...
        for ticker in tickers:
            ...
            for i, date in enumerate(date_range):
                ...
                volume = int(
                    2000000 + np.random.normal(0, 500000)
                )  # Higher volume for volatile stocks
    ...
    def create_benchmark_data(
        ...
        returns = np.random.normal(
            0.0003, 0.015, len(date_range)
        )  # ~0.03% daily return, 1.5% volatility
        prices = [base_price]
    ...
    def add_missing_data(
        data: pd.DataFrame, missing_dates: List[str], tickers: Optional[List[str]] = None
    ) -> pd.DataFrame:
        """
